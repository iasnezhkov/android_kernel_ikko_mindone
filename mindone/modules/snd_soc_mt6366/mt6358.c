			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	case MT6358_AUDENC_ANA_CON0:
		...
		break; /* D-3 (AUDIO-HANG, section E): used to crash in CON1 */
...
#endif
	/* do_exit(0) removed: dc_trim_thread is a kthread_run() threadfn --
	 * kernel/kthread.c's kthread() wrapper already calls do_exit(ret)
	 * after we return, so this was redundant (and do_exit is not
	 * exported for modules to call directly on this kernel anyway). */
	return 0;
}
