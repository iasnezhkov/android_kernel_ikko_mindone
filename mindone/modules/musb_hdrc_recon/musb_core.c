...
static int musb_probe(struct platform_device *pdev)
{
	 ...
	 */
	{
		...
		for (mi = 0; mi < pdev->num_resources; mi++)
			musb_res[mi] = pdev->resource[mi];

		mirq = platform_get_irq_byname(pdev, "mc");
		if (mirq > 0) {
			musb_res[mi].start = musb_res[mi].end = mirq;
			musb_res[mi].flags = IORESOURCE_IRQ;
			musb_res[mi].name = "mc";
			mi++;
		} else {
			dev_notice(&pdev->dev,
		...
		goto musb_irq_resources_done;
	}
skip_irq_fix:
	ret = platform_device_add_resources(musb_pdev,
				pdev->resource, pdev->num_resources);
	musb_irq_resources_done:
	;
