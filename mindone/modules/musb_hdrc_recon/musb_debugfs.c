...
{
	struct proc_dir_entry *e = proc_create_data(path, mode, NULL, ops, data);

	if (!e && !mindone_proc_dir) {
		mindone_proc_dir = proc_mkdir(PROC_DIR_MTK_USB, NULL);
		e = proc_create_data(path, mode, NULL, ops, data);
	}
	return e;
}
...
int musb_init_debugfs(struct musb *musb)
{
	...
	proc_files[idx] = mindone_proc_file(PROC_FILE_REGDUMP, 0444,
			&musb_regdump_fops, musb);
	if (!proc_files[idx]) {
		ret = -ENOMEM;
		goto err1;
	}
	...
	proc_files[idx] = mindone_proc_file(PROC_FILE_TESTMODE, 0644,
			&musb_test_mode_fops, musb);
	if (!proc_files[idx]) {
		ret = -ENOMEM;
		goto err1;
	}
	...
	proc_files[idx] = mindone_proc_file(PROC_FILE_REGW, 0644,
			&musb_regw_fops, musb);
	if (!proc_files[idx]) {
		ret = -ENOMEM;
		goto err1;
	...
	proc_files[idx] = mindone_proc_file(PROC_FILE_REGR, 0644,
			&musb_regr_fops, musb);
	if (!proc_files[idx]) {
		ret = -ENOMEM;
		goto err1;
	...
	proc_files[idx] = mindone_proc_file(PROC_FILE_SPEED, 0644,
			&musb_speed_fops, musb);
	if (!proc_files[idx]) {
		ret = -ENOMEM;
		goto err1;
	}
...
void musb_dr_debugfs_init(struct musb *musb)
{
	...
	proc_dr_files[idx++] = mindone_proc_file(PROC_FILE_MODE, 0644,
			&musb_mode_fops, musb);

	proc_dr_files[idx++] = mindone_proc_file(PROC_FILE_VBUS, 0644,
			&musb_vbus_fops, musb);
}
