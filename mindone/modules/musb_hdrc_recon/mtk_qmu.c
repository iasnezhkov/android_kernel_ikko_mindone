...
void qmu_reset_gpd_pool(u32 ep_num, u8 isRx)
{
	 ...
	 * re-read from the global array. Work off a local copy of the head, taken ONCE after the check. */
	{
		...
		if (isRx) {
			...
			memset_io(head, 0, size);
			Rx_gpd_end[ep_num] = Rx_gpd_last[ep_num] = head;
			/* one must be for tail */
			Rx_gpd_free_count[ep_num] = Rx_gpd_max_count[ep_num] - 1;
			TGPD_CLR_FLAGS_HWO(head);
			gpd_ptr_align(isRx, ep_num, head);
		} else {
			size = GPD_LEN_ALIGNED * Tx_gpd_max_count[ep_num];
			memset_io(head, 0, size);
			Tx_gpd_end[ep_num] = Tx_gpd_last[ep_num] = head;
			/* one must be for tail */
