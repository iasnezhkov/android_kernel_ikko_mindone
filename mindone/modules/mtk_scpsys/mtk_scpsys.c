...
/* ACK 6.1.175's dt-bindings/power/mt8192-power.h predates upstream's
 * MT8192_POWER_DOMAIN_ADSP addition (values 0-20, ending at CAM_RAWC) and
 * has no _NR sentinel. The vendor tree's mt8192-power.h is NOT a drop-in
 * replacement -- it renumbers the entire table differently (e.g. AUDIO/
 * CONN/ISP/IPE order differs), so copying it over would silently change
 * values 0-20 that this ACK header already defines. This mt8192 platform
 * table is dead code on real mt6789 hardware regardless (no mt6789
 * of_device_id entry in this generic multi-SoC mtk_scpsys.c), so the two
 * missing enumerators only need to be self-consistent within this file's
 * own local mt8192_scpsys_data[]/num_domains use -- appended locally
 * after the header's existing max (20) instead of editing the shared
 * tree header (grep-verified: nothing else in-tree references these two
 * names). */
