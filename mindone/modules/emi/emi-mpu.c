...
/* NOTE: no module_init() here - emi-mpu.c is linked into the combined
 * emi.ko together with emi-cen.o/emi-isu.o/emi-dbg.o. A loadable module
 * may only have ONE init_module symbol, so emimpu_init() is called
 * explicitly from emicen_init() (same __weak-declare + explicit-call
 * pattern already used there for emiisu_init()), instead of registering
 * its own module_init(). MODULE_DESCRIPTION/MODULE_LICENSE stay in
 * emi-cen.c for the combined module. */
