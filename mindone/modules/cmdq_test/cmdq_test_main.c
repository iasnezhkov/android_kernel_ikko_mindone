...
#include "mdp_engine_mt6789.h" /* k6 port: engine enum, needed directly */

/* k6 port: cmdq_core_reset_first_dump() has no definition anywhere in the
 * vendor platform tree or in kernel6/common61 (confirmed by exhaustive grep,
 * not just unexported -- genuinely absent). It only resets first-error-dump
 * bookkeeping between test iterations in this test/debug module, so a
 * local no-op stub is safe: worst case the dump is not cleared between
 * iterations. */
static void cmdq_core_reset_first_dump(void)
{
}
...
static struct timer_list timer_reqB; /* k6 port: forward decl, real decl below */
static bool test_timer_stop;

static void _testcase_sync_token_timer_func(struct timer_list *t)
{
	/* k6 port: timer_setup() carries no data payload any more. Recover
	 * the same per-instance token the pre-port code passed as "data" --
	 * every caller used CMDQ_SYNC_TOKEN_USER_0 except timer_reqB, which
	 * used CMDQ_SYNC_TOKEN_USER_1. */
	unsigned long data = (t == &timer_reqB) ?
		CMDQ_SYNC_TOKEN_USER_1 : CMDQ_SYNC_TOKEN_USER_0;
...
{
	/* k6 port: both call sites passed CMDQ_SYNC_TOKEN_USER_0. */
	unsigned long data = CMDQ_SYNC_TOKEN_USER_0;
...
static void testcase_sync_token(void)
{
	...
	do {
		...
		timer_setup(&test_timer, _testcase_sync_token_timer_func, 0);
...
static void testcase_async_request(void)
{
	...
	/* setup timer to trigger sync token */
	timer_setup(&timer_reqA, _testcase_sync_token_timer_func, 0);
	mod_timer(&timer_reqA, jiffies + msecs_to_jiffies(1000));

	timer_setup(&timer_reqB, _testcase_sync_token_timer_func, 0);
...
#define TEST_REQ_COUNT_ASYNC 9
	...
	timer_setup(&test_timer, _testcase_sync_token_timer_loop_func, 0);
	mod_timer(&test_timer, jiffies + msecs_to_jiffies(10));
...
static void testcase_async_request_partial_engine(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(scn); i++) {
		timer_setup(&timers[i], _testcase_sync_token_timer_func, 0);
...
static void testcase_sync_token_threaded(void)
{
	...
	/* setup timer to trigger sync token */
	for (i = 0; i < ARRAY_SIZE(scn); i++) {
		timer_setup(&eventTimer[i], _testcase_unlock_all_event_timer_func, 0);
...
static void testcase_loop(void)
{
	...
	timer_setup(&g_loopTimer, _testcase_loop_timer_func, 0);
	mod_timer(&g_loopTimer, jiffies + msecs_to_jiffies(300));
	cmdqCoreClearEvent(CMDQ_SYNC_TOKEN_USER_0);
...
static int testcase_cpu_config_mmsys(void *data)
{
	...
	cmdq_mdp_get_func()->mdpEnableCommonClock(true, 0);
	...
	cmdq_mdp_get_func()->mdpEnableCommonClock(false, 0);
	...
	struct random_data *data, u32 limit_size)
{
	...
	slot_idx = (get_random_u32() % data->slot_count) +
	...
	struct random_data *data, u32 limit_size)
{
	...
	bool use_mask = get_random_u32() % 10;

	if (use_mask) {
		data->mask = get_random_u32();
	...
	struct random_data *data, u32 limit_size)
{
	...
	if (data->thread->policy.poll_policy == CMDQ_TESTCASE_POLL_PASS)
		poll_bit = 1 << (get_random_u32() % 28);
	else
		poll_bit = 1 << (get_random_u32() % 32);
	...
	struct random_data *data, u32 limit_size, u32 *op_result)
{
	...
	condition_limit = get_random_u32() % (0xFFFF - data->expect_result);
	pre_op_count = get_random_u32() % remain_op_count + 1;
	post_op_count = remain_op_count > pre_op_count ?
		get_random_u32() % (remain_op_count - pre_op_count) : 0;
	branch_op = get_random_u32() % (u32)CMDQ_TESTCASE_BRANCH_MAX;
	...
	struct random_data *data, u32 limit_size, u32 *op_result)
{
	...
	while (ramain_op_count > min_op_count + reserve_op_count &&
		...
		/* condition value accept max 16 bit value */
		condition_limit = get_random_u32() % (0xFFFF - current_result -
			data->expect_result);
		condition_op = get_random_u32() % (u32)CMDQ_CONDITION_MAX;
		...
		if (op_if_counter == 0) {
			...
			branch_op = get_random_u32() % (u32)BRANCH_MAX;
		...
		logic_op_count = get_random_u32() % ramain_op_count + 1;
	...
	struct random_data *data, u32 limit_size)
{
	...
	/* make op count 4~N */
	limit_op_count = get_random_u32() % (limit_size /
		CMDQ_INST_SIZE - min_op_count);
	limit_op_count += min_op_count;

	if (!op_funcs[get_random_u32() % ARRAY_SIZE(op_funcs)](handle, data,
	...
	struct random_data *random_context, u32 limit_size)
{
	...
	while (handle->pkt->cmd_buf_size < limit_size) {
		...
		if (limit_size - handle->pkt->cmd_buf_size <=
			...
			total_func--;
		op_idx = get_random_u32() % total_func;
...
void _testcase_on_exec_suspend(struct cmdqRecStruct *task, s32 thread)
{
	...
	cmdqCoreSetEvent(tokens[get_random_u32() % ARRAY_SIZE(tokens)]);
}
	...
	struct workqueue_struct *release_queues[])
{
...
#ifdef CMDQ_SECURE_PATH_SUPPORT
	bool is_secure = (get_random_u32() % 20) == 0;
#endif
	...
	random_context->scenario =
		stress_scene[get_random_u32() % ARRAY_SIZE(stress_scene)];
	...
	case CMDQ_TESTCASE_WAITOP_RANDOM_NOTIMEOUT:
		...
		random_context->may_wait = get_random_u32() % 10;
		break;
	default:
		random_context->may_wait = get_random_u32() % 10;
		break;
	...
	for (i = 0; i < (get_random_u32() % max_buffer_count) + 1; i++) {
		u32 seed = get_random_u32() % ARRAY_SIZE(inst_count_pattern);
	...
	handle->pkt->priority = get_random_u32() % 16;
	if (thread_data->policy.engines_policy ==
		CMDQ_TESTCASE_ENGINE_RANDOM) {
		u64 rand_eng = 0;

		engine_sel = get_random_u32() %
			((1 << engine_cnt) - 1);
	...
	queue_work(release_queues[get_random_u32() % MAX_RELEASE_QUEUE],
		&random_context->release_work);

	msleep_interruptible(get_random_u32() %
...
static int _testcase_trigger_event_thread(void *data)
{
	...
	if (!trigger_interval)
		trigger_interval = get_random_u32() %
	...
	while (!atomic_read(&thread_data->stop)) {
		event_idx = get_random_u32() % ARRAY_SIZE(tokens);
		if (get_random_u32() % 2)
		...
		msleep_interruptible(get_random_u32() % trigger_interval + 1);
	}
...
void testmbox_dma_access(void)
{
	...
	va = (u32 *)cmdq_mbox_buf_alloc(clt, &slot);
	if (!va || !slot) {
	...
	cmdq_mbox_buf_free(clt, va, slot);
