...
/* No implementation exists anywhere in the reachable vendor/clean510v2
 * source trees for this k6 M=-module port -- these are ODM factory/
 * test-jig GPIO hooks (a vendor customization patch), unrelated to
 * this file's actual sm5602 fuel-gauge register map, bolted on as bare
 * extern decls with no matching .c anywhere. One of the 3 call sites in
 * tcpci_typec.c is already commented out by the vendor itself; the other
 * 2 are stubbed as no-ops here rather than guessing at GPIO numbers/
 * board wiring absent from every source tree on this VM. */
static inline void set_otg_en_t(int en) {}
static inline void test_gpio_t(int en) {}
