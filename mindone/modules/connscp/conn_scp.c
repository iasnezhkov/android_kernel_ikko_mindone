...
#include <drivers/misc/mediatek/conn_scp/conap_scp/conap_scp.h>
#include <linux/poll.h>

#include "conap_scp_priv.h"
#include <drivers/misc/mediatek/connectivity/common/connectivity_build_in_adapter.h>

/* MINDONE-GPSCONAP-CHIPINIT: the real caller (conninfra/consys_hw.c, detects
 * chip_info + emi_phy_addr at connsys power-on) is not in our tree. Neither
 * value is guessed: chip_info matches this SoC's own defconfig
 * (CONFIG_MTK_COMBO_CHIP_CONSYS_6789); emi_phy_addr is read verbatim from
 * THIS device's live devicetree reserved-memory block. The conap-specific
 * sub-offset within that block is genuinely unknown for chip 6789 (only
 * defined for 6893/6983/6895) - deliberately not guessed: conap_scp stays
 * cleanly disabled instead. Full detail: MINDONE-MODULES-NOTES-0901. */
#define MINDONE_GPS_CHIP_INFO		0x6789
#define MINDONE_GPS_EMI_PHY_ADDR	0x7E000000
...
int conn_scp_init(void)
{
	...
	/*****************************************/
	conap_scp_init();
	connectivity_export_conap_scp_init(MINDONE_GPS_CHIP_INFO, MINDONE_GPS_EMI_PHY_ADDR);
