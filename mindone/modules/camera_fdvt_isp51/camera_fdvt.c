...
void *aie_get_va(struct dma_buf *my_dma_buf)
{
	...
	int vmap_ret = dma_buf_vmap(my_dma_buf, &map);
 ...
 * hits anywhere in kernel6/common61). FDVT_ENQUE_REQ/FDVT_DEQUE_REQ transfer
 * a variable-length struct fdvt_config[] array through a pointer field
 * embedded in FDVT_Request, using copy_from_user()/copy_to_user() directly
 * against that pointer -- unlike camera_eeprom this never needed a
 * synthetic native-layout struct in user space, so there is no kmalloc
 * staging buffer here either. Splitting the ring-buffer business logic out
 * of the switch/case lets the compat handler below call it directly with a
 * compat_ptr()-translated pointer, instead of recursing through
 * filp->f_op->unlocked_ioctl() via compat_alloc_user_space().
 *
 * Bodies below are the original case bodies, unmodified, except that
 * "goto EXIT" (which used to target the shared error logger at the very
 * end of FDVT_ioctl) now targets a local "exit_business" label, since these
 * are now their own functions with their own scope.
 */
static signed int fdvt_enque_req_business(FDVT_Request fdvt_FdvtReq,
...
static signed int fdvt_deque_req_business(FDVT_Request *preq)
{
	signed int ret = 0;
	int idx, dequeNum;
	unsigned long flags;
	...
	spinlock_t *spinlock_lrq_ptr =
		&fdvt_info.spinlock_irq[FDVT_IRQ_TYPE_INT_FDVT_ST];
	FDVT_Request fdvt_FdvtReq = *preq;
...
*/
	...
	case FDVT_ENQUE_REQ:
		if (copy_from_user(&fdvt_FdvtReq, (void *)Param,
			...
			ret = fdvt_enque_req_business(fdvt_FdvtReq, pUserInfo);
		} else {
	...
	case FDVT_DEQUE_REQ:
		if (copy_from_user(&fdvt_FdvtReq, (void *)Param,
				   sizeof(FDVT_Request)) == 0) {
			ret = fdvt_deque_req_business(&fdvt_FdvtReq);

			/*
			 * fdvt_deque_req_business() never mutates
			 * m_pFdvtConfig, so re-checking it here after the
			 * call reproduces the original single-pass code's
			 * decision of whether to write the struct back to
			 * the caller (it skipped both copy_to_user() calls
			 * when the pointer was NULL, but still wrote the
			 * struct back when only the frame_config copy
			 * failed).
			 */
			if (fdvt_FdvtReq.m_pFdvtConfig) {
	...
	case COMPAT_FDVT_WRITE_REGISTER:
		{
			...
			if (err)
				return -EFAULT;
			RegIo.pData = (FDVT_REG_STRUCT *)compat_ptr(uptr);

			/*
			 * fdvt_read_reg()/fdvt_write_reg() already do their
			 * own get_user()/put_user()/copy_from_user() against
			 * RegIo.pData as a real __user pointer (translated
			 * above via compat_ptr()) and validate RegIo.count
			 * internally -- there is no synthetic native struct
			 * to build and no data to copy back into data32
			 * (count is never mutated by either helper).
			 */
			if (cmd == COMPAT_FDVT_READ_REGISTER)
				ret = fdvt_read_reg(&RegIo);
			else
				ret = fdvt_write_reg(&RegIo);
			return ret;
	...
	case COMPAT_FDVT_ENQUE_REQ:
		{
			...
			if (!filp->private_data)
				return -EFAULT;
			pUserInfo =
				(struct FDVT_USER_INFO_STRUCT *)filp->private_data;

			data32 = compat_ptr(arg);
			if (!data32)
				return -EFAULT;

			err = get_user(fdvt_FdvtReq.m_ReqNum, &data32->m_ReqNum);
			err |= get_user(uptr, &data32->m_pFdvtConfig);
			if (err)
				return -EFAULT;
			fdvt_FdvtReq.m_pFdvtConfig =
				(struct fdvt_config *)compat_ptr(uptr);
	...
	case COMPAT_FDVT_DEQUE_REQ:
		{
			...
			if (fdvt_FdvtReq.m_pFdvtConfig) {
				err = put_user(fdvt_FdvtReq.m_ReqNum,
					       &data32->m_ReqNum);
				if (err) {
					log_err("FDVT_DEQUE_REQ compat put_user failed\n");
					ret = -EFAULT;
				}
			}
			return ret;
...
int FDVT_pm_suspend(struct device *device)
{
	...
	/* pr_debug("calling %s()\n", __func__); */
	log_vrb("FDVT suspend clock_enable_count: %d, fdvt_count: %d",
...
int FDVT_pm_resume(struct device *device)
{
	...
	/* pr_debug("calling %s()\n", __func__); */
	log_vrb("FDVT resume clock_enable_count: %d, fdvt_count: %d",
		clock_enable_count, fdvt_count);
