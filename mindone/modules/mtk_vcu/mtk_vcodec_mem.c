...
{
	static struct vb2_queue vcu_dummy_vb2_queue;
	struct vb2_buffer vcu_dummy_vb = { .vb2_queue = &vcu_dummy_vb2_queue };

	return mem_ops->alloc(&vcu_dummy_vb, dev, size);
}
	...
	if (dbuf == NULL && src_vb != NULL)
		for (plane = 0; plane < src_vb->num_planes; plane++) {
			dma_addr = src_vb->vb2_queue->mem_ops->cookie(
				src_vb, src_vb->planes[plane].mem_priv);
	...
	if (dbuf == NULL && dst_vb != NULL)
		for (plane = 0; plane < dst_vb->num_planes; plane++) {
			dma_addr = dst_vb->vb2_queue->mem_ops->cookie(
				dst_vb, dst_vb->planes[plane].mem_priv);
			if (*dma_addr == mem_buff_data->iova) {
	...
	struct mem_obj *mem_buff_data)
{
	...
	vcu_buffer->mem_priv = vcu_mem_ops_alloc(vcu_queue->mem_ops, vcu_queue->dev,
		mem_buff_data->len);
	...
	cook = vcu_queue->mem_ops->vaddr(NULL, vcu_buffer->mem_priv);
	dma_addr = vcu_queue->mem_ops->cookie(NULL, vcu_buffer->mem_priv);
...
int vcu_buffer_flush_all(struct device *dev, struct mtk_vcu_queue *vcu_queue)
{
	...
	for (buffer = 0; buffer < num_buffers; buffer++) {
		...
		if (vcu_buffer->dbuf == NULL)
			dbuf = vcu_queue->mem_ops->get_dmabuf(NULL, vcu_buffer->mem_priv, flags);
		else
