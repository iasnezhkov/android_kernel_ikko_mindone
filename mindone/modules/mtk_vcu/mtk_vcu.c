				      ...
				      struct mem_obj *kobj, bool *out_copy_back)
{
	struct mtk_vcu_queue *vcu_queue =
		(struct mtk_vcu_queue *)file->private_data;
	struct mtk_vcu *vcu_dev = (struct mtk_vcu *)vcu_queue->vcu;
	struct device *dev = vcu_dev->dev;
	void *mem_priv;
	long ret;
	...
	*out_copy_back = false;
	...
	case VCU_SECURE_BUFFER_ALLOCATION:
		...
		if (cmd == VCU_MVA_ALLOCATION) {
			mem_priv = mtk_vcu_get_buffer(vcu_queue, kobj);
		} else if (cmd == VCU_SECURE_HANDLE_ALLOCATION) {
			mem_priv = mtk_vcu_get_sec_handle(vcu_queue, kobj);
		} else if (cmd == VCU_SECURE_BUFFER_ALLOCATION) {
			mem_priv = mtk_vcu_get_sec_buffer(vcu_dev->dev, vcu_queue, kobj);
			...
			mem_priv = mtk_vcu_get_page(vcu_queue, kobj);
		}
		mutex_unlock(&vcu_queue->dev_lock);
		if (IS_ERR_OR_NULL(mem_priv) == true) {
			kobj->va = (unsigned long)-1;
			kobj->pa = (unsigned long)-1;
			kobj->iova = (unsigned long)-1;
			*out_copy_back = true;
		...
		vcu_dbg_log("[VCU] ALLOCATION %d va %llx, pa %llx, iova %llx len %d\n",
			cmd == VCU_MVA_ALLOCATION, kobj->va,
			kobj->pa, kobj->iova, kobj->len);

		/* store map pa buffer type flag which will use in mmap*/
		if (cmd == VCU_PA_ALLOCATION)
			vcu_queue->map_buf_pa = kobj->pa + MAP_SHMEM_PA_BASE;
	...
	case VCU_PA_FREE:
		...
		if (cmd == VCU_MVA_FREE) {
			if (vcu_ptr->iommu_padding)
				kobj->iova |= 0x100000000UL;
			ret = mtk_vcu_free_buffer(vcu_queue, kobj);
			...
			ret = mtk_vcu_free_page(vcu_queue, kobj);
		}
		mutex_unlock(&vcu_queue->dev_lock);

		if (ret != 0L) {
			pr_info("[VCU] VCU_FREE failed %d va %llx, pa %llx, iova %llx\n",
				cmd == VCU_MVA_FREE, kobj->va,
				kobj->pa, kobj->iova);
			return -EINVAL;
		}

		vcu_dbg_log("[VCU] FREE %d va %llx, pa %llx, iova %llx\n",
			cmd == VCU_MVA_FREE, kobj->va, kobj->pa, kobj->iova);
	...
	case VCU_SECURE_HANDLE_FREE:
		...
		ret = mtk_vcu_free_sec_handle(vcu_queue, kobj);
	...
	case VCU_CACHE_INVALIDATE_BUFF:
		...
		ret = vcu_buffer_cache_sync(dev, vcu_queue,
			(dma_addr_t)kobj->iova, (size_t)kobj->len,
		...
		dev_dbg(dev, "[VCU] Cache flush buffer pa = %llx, size = %d\n",
			kobj->iova, (unsigned int)kobj->len);
...
/*
 * mtk_vcu_do_mem_ioctl() - native (64-bit) glue around
 * mtk_vcu_mem_ioctl_kernel(): copy the request in, run the shared logic,
 * copy the (possibly updated) struct back out -- matching the exact
 * copy-back rules of the original driver (see mtk_vcu_mem_ioctl_kernel()
 * comment above).
 */
static long mtk_vcu_do_mem_ioctl(struct file *file, unsigned int cmd,
				  unsigned char *user_data_addr)
{
	...
	kret = mtk_vcu_mem_ioctl_kernel(file, cmd, &mem_buff_data, &copy_back);

	if (kret != 0L) {
		/* Best-effort only: the original driver never checked this
		 * particular copy_to_user()'s result either.
		 */
		if (copy_back)
			(void)copy_to_user(user_data_addr, &mem_buff_data,
				(unsigned long)sizeof(struct mem_obj));
		return kret;
	}
				   ...
				   unsigned long arg)
{
	...
	case VCU_SECURE_BUFFER_FREE:
		user_data_addr = (unsigned char *)arg;
		ret = mtk_vcu_do_mem_ioctl(file, cmd, user_data_addr);
		break;
...
/*
 * compat_get_vpud_allocation_data() / compat_put_vpud_allocation_data() -
 * translate struct compat_mem_obj (32-bit caller layout) <-> struct mem_obj
 * (kernel-native layout) directly against a KERNEL-resident struct mem_obj.
 *
 * No compat_alloc_user_space() involved (removed upstream in 5.15): the get
 * side reads straight from the 32-bit caller's __user pointer into a kernel
 * local; the put side writes straight back out to that same __user pointer.
 * The shared business logic in mtk_vcu_mem_ioctl_kernel() never sees a user
 * pointer of either width.
 */
	...
	struct mem_obj *data)
{
	...
	err = put_user(data->iova, &data32->iova);
	err |= put_user(data->len, &data32->len);
	err |= put_user((compat_u64)data->pa, &data32->pa);
	err |= put_user((compat_u64)data->va, &data32->va);

	return (int)err;
}

/*
 * mtk_vcu_do_mem_ioctl_compat() - compat glue around
 * mtk_vcu_mem_ioctl_kernel(): translate the request in from the 32-bit
 * caller's struct compat_mem_obj, run the shared logic, translate the
 * (possibly updated) struct back out -- matching the exact copy-back rules
 * of the original driver (see mtk_vcu_mem_ioctl_kernel() comment).
 */
static long mtk_vcu_do_mem_ioctl_compat(struct file *file, unsigned int cmd,
					 struct compat_mem_obj __user *data32)
{
	struct mem_obj kobj;
	bool copy_back = false;
	...
	err = compat_get_vpud_allocation_data(data32, &kobj);
	if (err != 0)
		return err;

	ret = mtk_vcu_mem_ioctl_kernel(file, cmd, &kobj, &copy_back);

	if (copy_back) {
		err = compat_put_vpud_allocation_data(data32, &kobj);
		if (err != 0)
			return err;
	}
	return ret;
					  ...
					  unsigned long arg)
{
	...
	case COMPAT_VCU_SECURE_BUFFER_ALLOCATION:
		...
		if (cmd == COMPAT_VCU_MVA_ALLOCATION)
			native_cmd = VCU_MVA_ALLOCATION;
		else if (cmd == COMPAT_VCU_UBE_MVA_ALLOCATION)
			native_cmd = VCU_UBE_MVA_ALLOCATION;
		else if (cmd == COMPAT_VCU_SECURE_HANDLE_ALLOCATION)
			native_cmd = VCU_SECURE_HANDLE_ALLOCATION;
		else if (cmd == COMPAT_VCU_SECURE_BUFFER_ALLOCATION)
			native_cmd = VCU_SECURE_BUFFER_ALLOCATION;
		else
			native_cmd = VCU_PA_ALLOCATION;
		ret = mtk_vcu_do_mem_ioctl_compat(file, native_cmd, data32);
		break;
	...
	case COMPAT_VCU_SECURE_BUFFER_FREE:
		...
		if (cmd == COMPAT_VCU_MVA_FREE)
			native_cmd = VCU_MVA_FREE;
		else if (cmd == COMPAT_VCU_UBE_MVA_FREE)
			native_cmd = VCU_UBE_MVA_FREE;
		else if (cmd == COMPAT_VCU_SECURE_HANDLE_FREE)
			native_cmd = VCU_SECURE_HANDLE_FREE;
		else if (cmd == COMPAT_VCU_SECURE_BUFFER_FREE)
			native_cmd = VCU_SECURE_BUFFER_FREE;
		else
			native_cmd = VCU_PA_FREE;
		ret = mtk_vcu_do_mem_ioctl_compat(file, native_cmd, data32);
		break;
	...
	case COMPAT_VCU_CACHE_INVALIDATE_BUFF:
		data32 = compat_ptr((uint32_t)arg);
		native_cmd = (cmd == COMPAT_VCU_CACHE_FLUSH_BUFF) ?
			VCU_CACHE_FLUSH_BUFF : VCU_CACHE_INVALIDATE_BUFF;
		ret = mtk_vcu_do_mem_ioctl_compat(file, native_cmd, data32);
		break;
	default:
