				 ...
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		case MSDK_SCENARIO_ID_CUSTOM2:
		case MSDK_SCENARIO_ID_CUSTOM3:
			/* MINDONE fix (P127/P128, 12.09): first pass (previous
			 * commit) replaced the bogus vendor value 1465 with
			 * BINNING_AVERAGED(1), which fixed AE clamping (proved:
			 * mean-luma 7.4 -> 76.7 on image B26/b26-shot.jpg) but
			 * left a severe green cast: the SAME test photo that
			 * proves the brightness fix has meanR=1.2 meanG=128.7
			 * meanB=6.4 (G ~100x R, ~20x B) -- R and B are crushed
			 * to near-black while G renders normally, not a subtle
			 * tuning-table gap.
			 *
			 * Register evidence this driver's binning is genuine
			 * on-chip analog combination, not digital crop/scale:
			 * imx766_preview_setting / imx766_capture_30_setting /
			 * imx766_normal_video_setting / imx766_custom2_setting /
			 * imx766_custom3_setting all program 0x0900=0x01 (binning
			 * enable), 0x0901=0x44 (BINNING_TYPE H4/V4), full array
			 * 0x0348/0x034A=0x1FFF/0x17FF (8192x6144) cropped to
			 * output 0x034C/0x034E=0x0800/0x0600 (2048x1536) -- a
			 * true 4x4 (16:1) on-sensor combine, not a lighter 2x2
			 * quad-Bayer step.
			 *
			 * Disassembly of the stock 3A blobs (vendor_a.img
			 * /lib64/mt6789/, stockdump-0903) shows this same kernel
			 * value is NOT AE-only: both libcam.hal3a.v3.ae.so
			 * (NS3Av3::AeMgr::transBinSum/getBinSumTransSetting) and
			 * libcam.hal3a.v3.awb.so (NS3Av3::AAASensorMgr::
			 * getSensorBinningType/getMaximumBinSumRatio) call the
			 * identical sensor feature id (0x402a) to read it, so a
			 * wrong enum here can bias color math, not just exposure.
			 * AE's math is a closed-loop scalar (converges to the
			 * right brightness even fed an imperfect ratio, which is
			 * why BINNING_AVERAGED already "fixed" brightness); AWB's
			 * per-channel/black-level math is not self-correcting the
			 * same way, so it is the one still broken.
			 *
			 * A true 4x4 analog combine is physically a SUM of 16
			 * sub-pixel charges (that is what gives binning its
			 * sensitivity/SNR gain), not an average -- BINNING_
			 * AVERAGED undercounts both signal and black-level
			 * pedestal by the same factor the AWB path assumes was
			 * never added, consistent with R/B (which carry the
			 * AWB gain) being driven toward black while G (near-
			 * unity gain in gray-world AWB) stays normal. Switching
			 * to BINNING_SUMMED reports the readout MediaTek's own
			 * AE/AWB code actually expects from a 4x4 on-chip
			 * combine. Not yet re-verified with a live capture
			 * (device-free investigation, no camera access) --
			 * owner to confirm color on next boot.
			 */
