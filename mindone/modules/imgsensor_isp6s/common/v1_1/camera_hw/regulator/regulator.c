	...
	if (pregulator) {
		if (pin_state != IMGSENSOR_HW_PIN_STATE_LEVEL_0) {
			...
			if (regulator_enable(pregulator)) {
				...
				regulator_voltage[
				  pin_state - IMGSENSOR_HW_PIN_STATE_LEVEL_0]);
				pr_debug("MINDONE-CAM-PWR: regulator_set FAIL-enable sensor_idx=%d pin=%d uV=%d\n",
					sensor_idx, pin,
					regulator_voltage[
					pin_state - IMGSENSOR_HW_PIN_STATE_LEVEL_0]);
				return IMGSENSOR_RETURN_ERROR;
			}
			pr_debug("MINDONE-CAM-PWR: regulator_set OK sensor_idx=%d pin=%d uV=%d actual_uV=%d\n",
				sensor_idx, pin,
				regulator_voltage[
				pin_state - IMGSENSOR_HW_PIN_STATE_LEVEL_0],
			...
			if (regulator_disable(pregulator)) {
				PK_DBG(
					...
					pin);
				pr_debug("MINDONE-CAM-PWR: regulator_set FAIL-disable sensor_idx=%d pin=%d\n",
					sensor_idx, pin);
				return IMGSENSOR_RETURN_ERROR;
			}
			pr_debug("MINDONE-CAM-PWR: regulator_set OK-disable sensor_idx=%d pin=%d\n",
				sensor_idx, pin);
			atomic_dec(enable_cnt);
		}
	} else {
