 ...
 * hits anywhere in kernel6/common61). The three ioctls below used to build
 * a synthetic native-layout struct in user space via
 * compat_alloc_user_space(), fill it in from the 32-bit compat struct via
 * the compat_get_*()/compat_put_*() helpers above/below, and recurse
 * through filp->f_op->unlocked_ioctl() so the generic imgsensor_ioctl()
 * envelope (kmalloc(_IOC_SIZE(cmd)) + copy_from_user() + dispatch to
 * adopt_CAMERA_HW_*() + optional copy_to_user()) would do the real work
 * against that synthetic struct.
 *
 * adopt_CAMERA_HW_GetInfo2()/Control()/FeatureControl() all take a plain
 * kernel buffer (void *) and never reassign the pointer fields inside it
 * (pInfo/pSensorResolution, pImageWindow/pSensorConfigData,
 * pFeaturePara/pFeatureParaLen) -- they only read/write through them via
 * copy_from_user()/copy_to_user()/get_user()/put_user() against whatever
 * real __user address is already in the field ("Assume pointer is not
 * change", per the original compat_put_*() comments below). So instead of
 * faking a user-space buffer, kmalloc() the kernel-native struct directly
 * here, translate the compat struct's fields into it (compat_ptr() for the
 * pointer fields, direct get_user() for the scalars), and call the
 * adopt_CAMERA_HW_*() business function directly -- no double indirection
 * through the ioctl vtable. Scalar fields that the original compat_put_*()
 * helpers copied back (InvokeCamera/FeatureId, InvokeCamera/ScenarioId,
 * SensorId) are put_user()'d back the same way after the call, in case the
 * business function mutated them; the pointer fields are not, matching the
 * original "assume pointer is not changed" behavior.
 */
	...
	unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_KDIMGSENSORIOC_X_FEATURECONCTROL:
	{
		...
		pBuf->pFeaturePara = (typeof(pBuf->pFeaturePara))compat_ptr(p);
		err |= get_user(p, &data32->pFeatureParaLen);
		pBuf->pFeatureParaLen =
			(typeof(pBuf->pFeatureParaLen))compat_ptr(p);
		if (err) {
			kfree(pBuf);
			return -EFAULT;
		}

		ret = adopt_CAMERA_HW_FeatureControl(pBuf);

		err = put_user(pBuf->InvokeCamera, &data32->InvokeCamera);
		err |= put_user(pBuf->FeatureId, &data32->FeatureId);
		kfree(pBuf);
		if (err != 0) {
			PK_PR_ERR(
	...
	case COMPAT_KDIMGSENSORIOC_X_CONTROL:
		{
			...
			pBuf->pImageWindow =
				(typeof(pBuf->pImageWindow))compat_ptr(p);
			err |= get_user(p, &data32->pSensorConfigData);
			pBuf->pSensorConfigData =
				(typeof(pBuf->pSensorConfigData))compat_ptr(p);
			if (err) {
				kfree(pBuf);
				return -EFAULT;
			}

			ret = adopt_CAMERA_HW_Control(pBuf);

			err = put_user(pBuf->InvokeCamera, &data32->InvokeCamera);
			err |= put_user(pBuf->ScenarioId, &data32->ScenarioId);
			kfree(pBuf);
			if (err != 0)
				PK_PR_ERR(
	...
	case COMPAT_KDIMGSENSORIOC_X_GETINFO2:
		{
			...
			pBuf->pInfo = (typeof(pBuf->pInfo))compat_ptr(p);
			err |= get_user(p, &data32->pSensorResolution);
			pBuf->pSensorResolution =
				(typeof(pBuf->pSensorResolution))compat_ptr(p);
			if (err) {
				kfree(pBuf);
				return -EFAULT;
			}

			ret = adopt_CAMERA_HW_GetInfo2(pBuf);

			err = put_user(pBuf->SensorId, &data32->SensorId);
			kfree(pBuf);
			if (err != 0)
				PK_PR_ERR(
