...
static inline void Disable_Unprepare_ccf_clock(void)
{
	...
	if (dip_clk.DIP_IMG_MFB_DIP != NULL || dip_clk.DIP_IMG_DIP2 != NULL) {
		...
		LOG_INF("mtk_smi_larb_put larb11\n");
	}
...
#if defined(CONFIG_COMPAT) && !defined(MINDONE_NO_COMPAT_IOCTL)
			...
			unsigned long pm_event, void *unused)
{
	...
	case PM_SUSPEND_PREPARE: /*enter suspend*/
		...
		if (g_DIP_PMState == 0) {
			LOG_VRB("DIP suspend G_u4DipEnClkCnt: %d, g_u4DipCnt: %d",
				G_u4DipEnClkCnt,
				g_u4DipCnt);
			g_DIP_PMState = 1;
		}
	...
	case PM_POST_SUSPEND:    /*after resume*/
		...
		if (g_DIP_PMState == 1) {
			LOG_VRB("DIP resume G_u4DipEnClkCnt: %d, g_u4DipCnt: %d",
				G_u4DipEnClkCnt,
				g_u4DipCnt);
			g_DIP_PMState = 0;
		}
