...
#include <asm/setup.h>/*drv modify for linear vibrator cali begin*/
#include "haptic_hv.h"
...
#else
#ifdef KERNEL_OVER_5_10
	aw_haptic->vib_dev.name = "vibrator";//drv-modify-pengzhipeng-202230211
#else
	aw_haptic->vib_dev.name = "vibrator";
#endif
#endif
...
const char *haptic_get_cmd(void)
{
	...
	if (of_chosen) {
		...
		if (!bootargs)
			...
			strncpy(__haptic_cmdline, bootargs, 600);
			printk("%s: bootargs: %s\n", __func__, bootargs);
		}
	} else
...
static int haptic_get_cmdline_cali_data(void)
{
	char *ptr = NULL;
	int cali_data = 0;
	ptr = strstr(haptic_get_cmd(),"aw_cali=");
	if(ptr == NULL)
	{
...
/* 6.6+: single-argument probe (see MINDONE_I2C_PROBE) */
