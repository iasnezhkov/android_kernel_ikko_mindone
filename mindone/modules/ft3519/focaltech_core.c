...
#include <mindone/compat.h>	/* 6.12: of_get_named_gpio_flags, i2c probe */
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    /* reset, irq gpio info */
    pdata->reset_gpio = mindone_of_get_named_gpio_flags(np, "focaltech,reset-gpio",
                        0, &pdata->reset_gpio_flags);
    if (pdata->reset_gpio < 0)
        FTS_ERROR("Unable to get reset_gpio");

    pdata->irq_gpio = mindone_of_get_named_gpio_flags(np, "focaltech,irq-gpio",
...
/* i2c_driver.remove changed int->void upstream (same real driver-core
 * cleanup already fixed elsewhere in this repo). */
...
static struct i2c_driver fts_ts_driver = {
    MINDONE_I2C_PROBE(fts_ts_probe),
