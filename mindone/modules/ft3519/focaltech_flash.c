...
static int fts_read_file(char *file_name, u8 **file_buf)
{
    ...
    *file_buf = (u8 *)vmalloc(file_len);
    ...
    /* mm_segment_t/get_fs()/set_fs()/KERNEL_DS were removed upstream
     * (the whole 'fake address space' trick existed only to let
     * vfs_read()-style calls accept a kernel buffer). The real read
     * below already uses kernel_read(), which takes a kernel buffer
     * directly and needs no address-space override at all -- that is
     * the documented reason kernel_read()/kernel_write() were added,
     * so this bracket was already vestigial even on the vendor's own
     * target kernel. Dropping it changes no behavior. */
    pos = 0;
