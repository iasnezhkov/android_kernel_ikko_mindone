...
/*
 * struct pwm_ops.config/.enable/.disable were removed upstream in favor of
 * a single atomic .apply(chip, pwm, state) callback (real, well-documented
 * PWM subsystem migration that most upstream PWM drivers went through
 * ~5.14-6.x). This merges the exact same 3 real functions above (unchanged)
 * behind the new entry point -- same logic, just called from one place:
 * config+enable when state->enabled is requested, disable when it is not.
 * Truncating u64 period/duty_cycle (nanoseconds) to int matches what
 * mtk_disp_pwm_config() itself already expected (its own params are
 * "int duty_ns, int period_ns"), so this is not a new range assumption.
 */
			      ...
			      const struct pwm_state *state)
{
	...
	if (!state->enabled) {
		mtk_disp_pwm_disable(chip, pwm);
		return 0;
	}

	err = mtk_disp_pwm_config(chip, pwm, (int)state->duty_cycle,
				  (int)state->period);
	if (err)
		return err;

	return mtk_disp_pwm_enable(chip, pwm);
}
