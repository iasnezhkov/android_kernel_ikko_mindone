...
static int lpm_cpuidle_prepare(struct cpuidle_driver *drv, int index)
{
	...
	if (lpm_generic_rcu_idle(drv, index))
		ct_idle_exit(); /* rcu_idle_exit() renamed upstream, common61/kernel/context_tracking.c:333 */
	...
	if (lpm_generic_rcu_idle(drv, index))
		ct_idle_enter(); /* rcu_idle_enter() renamed upstream, common61/kernel/context_tracking.c:317 */
