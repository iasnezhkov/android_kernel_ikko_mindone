...
/* ACK 6.1.175 android_debug_symbols.h enum has no ADS_NR_SWAP_PAGES slot
 * (a vendor-only addition). Map it to ADS_END (the bounds-check sentinel):
 * android_debug_symbol() itself does "if (symbol >= ADS_END) return
 * ERR_PTR(-EINVAL);" before ever touching the fixed-size ads_entries[]
 * array in vmlinux, so this is *exactly* the already-supported
 * "symbol unavailable" path -- caller already IS_ERR()-checks both at
 * init and at use (see mmstat_available_swap_pages below). Deliberately
 * NOT editing the shared android_debug_symbols.h enum: inserting a new
 * member there would shift ADS_END past the end of vmlinux's already-
 * linked ads_entries[ADS_END] array (real OOB read at runtime), since
 * vmlinux is not being relinked in this session. */
#define ADS_NR_SWAP_PAGES ADS_END
