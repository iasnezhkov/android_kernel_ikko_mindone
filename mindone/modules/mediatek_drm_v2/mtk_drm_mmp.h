...
#include "mmprofile.h"
/* mmprofile_function.h intentionally NOT included: it independently
 * redeclares (and, when CONFIG_MMPROFILE is disabled, redefines as
 * static inline) the exact same mmprofile_* function set as mmprofile.h
 * -- confirmed a strict subset (mmprofile.h has every function
 * mmprofile_function.h has, plus mmprofile_get_dump_size). Real vendor-tree
 * bug: mtk_drm_mmp.h originally included both, which is always a
 * redefinition conflict regardless of CONFIG_MMPROFILE state (both
 * headers duplicate their own #if-enabled and #else-disabled branches
 * for the same names). Dropping the redundant include, not any real
 * declaration -- mmprofile.h alone provides everything this file uses. */
