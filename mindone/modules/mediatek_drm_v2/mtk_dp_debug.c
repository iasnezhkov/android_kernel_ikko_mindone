...
#include <drivers/gpu/drm/mediatek/mediatek_v2/mtk_dp_debug.h>
...
void mtk_dp_debug(const char *opt)
{
	...
	if (strncmp(opt, "fakecablein:", 12) == 0) {
		...
		ret = sscanf(opt, "adjust_phy:%hhu,%hhu,%hhu\n", &index, &c0, &cp1);
		if (ret != 3) {
			DPTXERR("ret = %d\n", ret);
			return;
		}
		...
		ret = sscanf(opt, "pattern:%hhu,%hhu\n", &enable, &resolution);
		if (ret != 2) {
			DPTXMSG("ret = %d\n", ret);
			return;
		}
		...
		ret = sscanf(opt, "maxlinkrate:%hhu,%hhu\n", &enable, &maxlinkrate);
		if (ret != 2) {
			DPTXMSG("ret = %d\n", ret);
			return;
		}
