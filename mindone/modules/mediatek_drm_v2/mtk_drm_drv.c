...
MODULE_PARM_DESC(mindone_log, "print MINDONE diagnostic markers (0 = silent)");
#include <drm/drm_crtc_helper.h>
#include <drm/drm_probe_helper.h>
#include <drm/drm_gem.h>
#include <drm/drm_gem_dma_helper.h> /* renamed from drm_gem_cma_helper.h, real ~6.x GEM CMA->DMA helper rename */
#include <drm/drm_fourcc.h>
#include <drm/drm_framebuffer.h> /* full struct drm_framebuffer def, needed for ->format access */
...
#include <drivers/gpu/drm/mediatek/mediatek_v2/mtk_drm_debugfs.h>
...
#include <drivers/gpu/drm/mediatek/mediatek_v2/mtk_disp_color.h>
#include <drivers/gpu/drm/mediatek/mediatek_v2/mtk_disp_gamma.h>
#include <drivers/gpu/drm/mediatek/mediatek_v2/mtk_disp_aal.h>
#include "mtk_disp_c3d.h"
#include <drivers/gpu/drm/mediatek/mediatek_v2/mtk_disp_chist.h>
...
#include <drivers/gpu/drm/mediatek/mediatek_v2/mtk_dp_api.h>
//#include "swpm_me.h"
...
unsigned int mindone_lk_islcmfound(void)
{
	...
	return videolfb_tag ? videolfb_tag->islcmfound : 0;
}

bool mindone_lk_display_alive(void)
{
	return mindone_lk_alive == 1;
}

void mindone_lk_set_alive(int alive)
{
	mindone_lk_alive = alive;
	mindone_lk_alive_ro = alive;
}
...
static bool mtk_drm_is_enable_from_lk(struct drm_crtc *crtc)
{
	...
	if (drm_crtc_index(crtc) == 0)
		...
		return true;
#else
		return mindone_lk_display_alive();
#endif
	return false;
}
				...
				struct drm_atomic_state *state)
{
	...
	MINDONE_PR("MINDONE-AC: atomic-complete begin state=%px\n", state);
	if (mtk_atomic_skip_config(state)) {
		drm_atomic_helper_commit_planes(drm, state,
			DRM_PLANE_COMMIT_ACTIVE_ONLY);
		MINDONE_PR("MINDONE-AC: atomic-complete end (skip-config)\n");
		return;
	}
	MINDONE_PR("MINDONE-AC: complete-step wait-for-fences\n");
	 ...
	 */
	MINDONE_PR("MINDONE-AC: complete-step doze-preparation\n");
	mtk_atomic_doze_preparation(drm, state);

	MINDONE_PR("MINDONE-AC: complete-step modeset-disables\n");
	drm_atomic_helper_commit_modeset_disables(drm, state);
	MINDONE_PR("MINDONE-AC: complete-step modeset-enables\n");
	drm_atomic_helper_commit_modeset_enables(drm, state);
	MINDONE_PR("MINDONE-AC: complete-step post-modeset-enables\n");

	mtk_set_first_config(drm, state);

	MINDONE_PR("MINDONE-AC: complete-step enable-trig\n");
	mtk_drm_enable_trig(drm, state);
	MINDONE_PR("MINDONE-AC: complete-step post-enable-trig\n");
	...
	MINDONE_PR("MINDONE-AC: complete-step mml\n");
	mtk_atomic_mml(drm, state);
	MINDONE_PR("MINDONE-AC: complete-step post-mml\n");

	if (!mtk_atomic_skip_plane_update(private, state)) {
		MINDONE_PR("MINDONE-AC: complete-step commit-planes\n");
		drm_atomic_helper_commit_planes(drm, state,
						DRM_PLANE_COMMIT_ACTIVE_ONLY);
		MINDONE_PR("MINDONE-AC: complete-step post-commit-planes\n");
	...
	if (!mtk_drm_helper_get_opt(private->helper_opt,
		...
		MINDONE_PR("MINDONE-AC: complete-step wait-for-vblanks\n");
		drm_atomic_helper_wait_for_vblanks(drm, state);
		MINDONE_PR("MINDONE-AC: complete-step post-wait-for-vblanks\n");
	}

	drm_atomic_helper_cleanup_planes(drm, state);
	MINDONE_PR("MINDONE-AC: atomic-complete end\n");
			    ...
			    struct drm_atomic_state *state)
{
	...
	MINDONE_PR("MINDONE-AC: atomic-check begin state=%px\n", state);
	...
	for_each_new_crtc_in_state(state, crtc, crtc_state, i) {
		...
		if (ret) {
			DDPINFO("DRM add conn failed! state:%p, ret:%d\n",
					state, ret);
			MINDONE_PR("MINDONE-AC: atomic-check end ret=%d (add-conn-fail)\n", ret);
			return ret;
		}
	}

	MINDONE_PR("MINDONE-AC: atomic-check end ret=%d\n", ret);
	return ret;
}
			     ...
			     struct drm_atomic_state *state, bool async)
{
	...
	MINDONE_PR("MINDONE-AC: atomic-commit begin async=%d state=%px\n", async, state);
	DDP_PROFILE("[PROFILE] %s+\n", __func__);
	ret = drm_atomic_helper_prepare_planes(drm, state);
	if (ret) {
		MINDONE_PR("MINDONE-AC: atomic-commit end ret=%d (prepare-planes-fail)\n", ret);
		return ret;
	}
...
commit_unlock:
	...
	MINDONE_PR("MINDONE-AC: atomic-commit end ret=0\n");
	return 0;
}
...
static unsigned long mindone_top_clk_isr_race_hits;
module_param(mindone_top_clk_isr_race_hits, ulong, 0444);
MODULE_PARM_DESC(mindone_top_clk_isr_race_hits,
	"mtk_drm_top_clk_disable_unprepare() giving up with top_isr_ref still > 0 — count");
	...
	if (chosen_node) {
		...
		if (videolfb_tag) {
			...
			*fps = videolfb_tag->fps;
			...
			DDPMSG("MINDONE-LK-TAG: atag,videolfb absent\n");
			return -1;
		}
	} else {
		DDPMSG("MINDONE-LK-TAG: /chosen absent\n");
		return -1;
	}
			   ...
			   struct drm_file *file_priv)
{
	...
	MINDONE_PR("MINDONE-RT: get-info-ioctl session_id=0x%08x s_type=%d\n",
		info->session_id, s_type);
			 ...
			 struct drm_file *file_priv)
{
	...
	MINDONE_PR("MINDONE-RT: set-ddp-mode session_mode=%u\n", *session_mode);
...
static int mtk_drm_kms_init(struct drm_device *drm)
{
	...
	/* allow_fb_modifiers removed upstream -- modifiers are allowed by
	 * default now, opt-OUT via the new (inverted) fb_modifiers_not_supported
	 * flag, which we deliberately leave at its false default to match the
	 * original allow_fb_modifiers=true intent. */
	 ...
	 * CONFIG_DRM_LEGACY (confirmed: drm_vblank.c's
	 * drm_wait_vblank_supported() only reads it inside an
	 * IS_ENABLED(CONFIG_DRM_LEGACY) block guarding DRIVER_LEGACY
	 * drivers; the modern path this ATOMIC/MODESET driver actually
	 * takes is drm_dev_has_vblank(dev), unaffected by this field).
	 * The comment above described a legacy-midlayer workaround that
	 * no longer applies -- dropping the now-nonexistent assignment.
	 */
...
{ \
	int __ret; \
	MINDONE_PR("MINDONE-IOC: enter cmd=0x%08x nr=0x%02x pid=%d name=%s\n", \
		(unsigned int)DRM_IOCTL_##IOCTL, (unsigned int)DRM_##IOCTL, \
		current->pid, #IOCTL); \
	__ret = fn(dev, data, file_priv); \
	MINDONE_PR("MINDONE-IOC: exit  cmd=0x%08x nr=0x%02x ret=%d pid=%d name=%s\n", \
		(unsigned int)DRM_IOCTL_##IOCTL, (unsigned int)DRM_##IOCTL, \
		__ret, current->pid, #IOCTL); \
	return __ret; \
}

MRK_WRAP(MTK_GEM_CREATE, mtk_gem_create_ioctl)
MRK_WRAP(MTK_GEM_MAP_OFFSET, mtk_gem_map_offset_ioctl)
MRK_WRAP(MTK_GEM_SUBMIT, mtk_gem_submit_ioctl)
MRK_WRAP(MTK_SESSION_CREATE, mtk_drm_session_create_ioctl)
MRK_WRAP(MTK_SESSION_DESTROY, mtk_drm_session_destroy_ioctl)
MRK_WRAP(MTK_LAYERING_RULE, mtk_layering_rule_ioctl)
MRK_WRAP(MTK_CRTC_GETFENCE, mtk_drm_crtc_getfence_ioctl)
MRK_WRAP(MTK_CRTC_GETSFFENCE, mtk_drm_crtc_get_sf_fence_ioctl)
MRK_WRAP(MTK_SET_MSYNC_PARAMS, mtk_drm_set_msync_params_ioctl)
MRK_WRAP(MTK_GET_MSYNC_PARAMS, mtk_drm_get_msync_params_ioctl)
MRK_WRAP(MTK_WAIT_REPAINT, mtk_drm_wait_repaint_ioctl)
MRK_WRAP(MTK_GET_DISPLAY_CAPS, mtk_drm_get_display_caps_ioctl)
MRK_WRAP(MTK_SET_DDP_MODE, mtk_drm_set_ddp_mode)
MRK_WRAP(MTK_GET_SESSION_INFO, mtk_drm_get_info_ioctl)
MRK_WRAP(MTK_GET_MASTER_INFO, mtk_drm_get_master_info_ioctl)
MRK_WRAP(MTK_PQ_PERSIST_PROPERTY, mtk_drm_ioctl_pq_get_persist_property)
MRK_WRAP(MTK_SET_CCORR, mtk_drm_ioctl_set_ccorr)
MRK_WRAP(MTK_CCORR_EVENTCTL, mtk_drm_ioctl_ccorr_eventctl)
MRK_WRAP(MTK_CCORR_GET_IRQ, mtk_drm_ioctl_ccorr_get_irq)
MRK_WRAP(MTK_AIBLD_CV_MODE, mtk_drm_ioctl_aibld_cv_mode)
MRK_WRAP(MTK_SUPPORT_COLOR_TRANSFORM, mtk_drm_ioctl_support_color_matrix)
MRK_WRAP(MTK_SET_GAMMALUT, mtk_drm_ioctl_set_gammalut)
MRK_WRAP(MTK_SET_12BIT_GAMMALUT, mtk_drm_ioctl_set_12bit_gammalut)
MRK_WRAP(MTK_SET_PQPARAM, mtk_drm_ioctl_set_pqparam)
MRK_WRAP(MTK_SET_PQINDEX, mtk_drm_ioctl_set_pqindex)
MRK_WRAP(MTK_SET_COLOR_REG, mtk_drm_ioctl_set_color_reg)
MRK_WRAP(MTK_MUTEX_CONTROL, mtk_drm_ioctl_mutex_control)
MRK_WRAP(MTK_READ_REG, mtk_drm_ioctl_read_reg)
MRK_WRAP(MTK_WRITE_REG, mtk_drm_ioctl_write_reg)
MRK_WRAP(MTK_BYPASS_COLOR, mtk_drm_ioctl_bypass_color)
MRK_WRAP(MTK_PQ_SET_WINDOW, mtk_drm_ioctl_pq_set_window)
MRK_WRAP(MTK_WRITE_SW_REG, mtk_drm_ioctl_write_sw_reg)
MRK_WRAP(MTK_READ_SW_REG, mtk_drm_ioctl_read_sw_reg)
MRK_WRAP(MTK_GET_LCM_INDEX, mtk_drm_ioctl_get_lcm_index)
MRK_WRAP(MTK_GET_PANELS_INFO, mtk_drm_ioctl_get_all_connector_panel_info)
MRK_WRAP(MTK_AAL_INIT_REG, mtk_drm_ioctl_aal_init_reg)
MRK_WRAP(MTK_AAL_GET_HIST, mtk_drm_ioctl_aal_get_hist)
MRK_WRAP(MTK_AAL_SET_PARAM, mtk_drm_ioctl_aal_set_param)
MRK_WRAP(MTK_AAL_EVENTCTL, mtk_drm_ioctl_aal_eventctl)
MRK_WRAP(MTK_AAL_INIT_DRE30, mtk_drm_ioctl_aal_init_dre30)
MRK_WRAP(MTK_AAL_GET_SIZE, mtk_drm_ioctl_aal_get_size)
MRK_WRAP(MTK_HDMI_GET_DEV_INFO, mtk_drm_dp_get_dev_info)
MRK_WRAP(MTK_HDMI_AUDIO_ENABLE, mtk_drm_dp_audio_enable)
MRK_WRAP(MTK_HDMI_AUDIO_CONFIG, mtk_drm_dp_audio_config)
MRK_WRAP(MTK_HDMI_GET_CAPABILITY, mtk_drm_dp_get_cap)
MRK_WRAP(MTK_MML_GEM_SUBMIT, mtk_drm_ioctl_mml_gem_submit)
MRK_WRAP(MTK_SET_DISP_TDSHP_REG, mtk_drm_ioctl_tdshp_set_reg)
MRK_WRAP(MTK_DISP_TDSHP_GET_SIZE, mtk_drm_ioctl_tdshp_get_size)
MRK_WRAP(MTK_C3D_GET_BIN_NUM, mtk_drm_ioctl_c3d_get_bin_num)
MRK_WRAP(MTK_C3D_GET_IRQ, mtk_drm_ioctl_c3d_get_irq)
MRK_WRAP(MTK_C3D_EVENTCTL, mtk_drm_ioctl_c3d_eventctl)
MRK_WRAP(MTK_C3D_SET_LUT, mtk_drm_ioctl_c3d_set_lut)
MRK_WRAP(MTK_SET_BYPASS_C3D, mtk_drm_ioctl_bypass_c3d)
MRK_WRAP(MTK_GET_CHIST, mtk_drm_ioctl_get_chist)
MRK_WRAP(MTK_GET_CHIST_CAPS, mtk_drm_ioctl_get_chist_caps)
MRK_WRAP(MTK_SET_CHIST_CONFIG, mtk_drm_ioctl_set_chist_config)
MRK_WRAP(MTK_SET_DITHER_PARAM, mtk_drm_ioctl_set_dither_param)
MRK_WRAP(MTK_BYPASS_DISP_GAMMA, mtk_drm_ioctl_bypass_disp_gamma)
MRK_WRAP(MTK_FACTORY_LCM_AUTO_TEST, mtk_drm_fm_lcm_auto_test)
MRK_WRAP(MTK_GET_PQ_CAPS, mtk_drm_ioctl_get_pq_caps)
MRK_WRAP(MTK_SET_PQ_CAPS, mtk_drm_ioctl_set_pq_caps)
MRK_WRAP(MTK_SEC_HND_TO_GEM_HND, mtk_drm_sec_hnd_to_gem_hnd)
#undef MRK_WRAP
...
static struct drm_driver mtk_drm_driver = {
	...
	/*
	 * gem_free_object_unlocked / gem_vm_ops / gem_prime_export /
	 * gem_prime_get_sg_table removed from struct drm_driver upstream
	 * (real GEM per-object funcs-table restructuring, ~5.x) -- now live
	 * on struct drm_gem_object_funcs (see mtk_drm_gem_object_funcs in
	 * mtk_drm_gem.c, wired to obj->funcs at object-init time). export
	 * was just the generic drm_gem_prime_export -- drm_prime.c already
	 * falls back to calling it directly when funcs->export is unset, so
	 * dropping the field changes nothing behaviorally.
	 *
	 * dumb_destroy removed too: current drm_drv.h says outright "GEM
	 * based drivers must not overwrite this" -- the default
	 * implementation (drm_gem_dumb_destroy) is used automatically when
	 * the field is left unset, matching the real vendor intent (it was
	 * only ever pointed at the stock function, never a custom one).
	 */
