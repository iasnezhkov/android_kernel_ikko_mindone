...
/*
 * k6 bring-up (k6-schedh-port): this module used to define its own
 * sched_feat_keys[]/__SCHED_FEAT_* via a local features.h X-macro copy that
 * had drifted from common61's real kernel/sched/features.h (missing
 * HRTICK_DL/SIS_UTIL/LATENCY_WARN/ALT_PERIOD/BASE_SLICE, had a stale
 * SIS_AVG_CPU). common61's kernel/sched/sched.h (now reachable via the -I
 * redirect above) already declares sched_feat_keys[] (EXPORT_SYMBOL_GPL'd in
 * kernel/sched/debug.c) and provides the real sched_feat() macro this file
 * actually uses (sched_feat(UTIL_EST), below) -- the local copy was a
 * redefinition of an already-available real symbol, not a genuine need.
 */
