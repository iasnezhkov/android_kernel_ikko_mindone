...
/*
 * BUF_PRIV_MAX_CNT: size of the per-buffer per-IOMMU-domain bookkeeping
 * arrays (mapped[]/dev_info[]/mapped_table[] below). Not declared ANYWHERE in
 * the vendor tree either -- mtk_mm_heap.c only ever *uses* it, never defines
 * it (checked across multiple vendor platform trees for this SoC family). The
 * vendor's sibling driver drivers/staging/android/ion/heaps/mtk_ion_mm_heap.c:36-38
 * sizes the structurally identical mapped[]/dev_class[]/table[] arrays with
 * MTK_M4U_DOM_NR_MAX, which is the same "one array slot per IOMMU domain"
 * bound this file needs -- alias it here.
 */
...
static int mtk_mm_heap_vmap(struct dma_buf *dmabuf, struct iosys_map *map)
{
	struct mtk_mm_heap_buffer *buffer = dmabuf->priv;
	void *vaddr;
	int ret = 0;
...
static void mtk_mm_heap_vunmap(struct dma_buf *dmabuf, struct iosys_map *map)
{
	struct mtk_mm_heap_buffer *buffer = dmabuf->priv;
