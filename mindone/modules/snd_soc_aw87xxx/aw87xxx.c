...
exit_dtsi_parse_failed:
	...
	/* rst_gpio was obtained via devm_gpio_request_one() above, so it is
	 * already devm-managed -- the real devm framework guarantees it is
	 * released automatically on this probe-failure unwind / device
	 * detach. devm_gpio_free() itself was removed from this kernel's
	 * legacy gpio.h (devm_gpio_request_one() is still real and present,
	 * confirmed) -- dropping this early manual free changes no real
	 * behavior, the resource is still freed via the guaranteed devm
	 * path, just not this instant. */
