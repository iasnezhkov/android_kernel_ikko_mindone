...
static void vow_service_Init(void)
{
	...
	if (init_flag != 1) {
		...
		audio_task_register_callback(TASK_SCENE_VOW,
					     vow_IPICmd_Received);
	/* vow_Task_Unloaded_Handling() was a debug-log no-op tied to an
	 * unload callback slot audio_task_register_callback() no longer
	 * accepts (vendor header confirms 2-arg signature) -- nothing to
	 * port, the slot itself is gone from this API generation. */
		/*Initialization*/
...
#if IS_ENABLED(CONFIG_COMPAT) && !defined(MINDONE_NO_COMPAT_IOCTL)
...
/* Accessor for vow_scp.c (ver02-derived): the real vow_scp.c calls this to
 * decide whether to bypass an SCP IPI send while SCP is mid-recovery.
 * ver01 (which this vow.c is based on) checked vowserv.scp_recovering
 * inline instead of through a function -- MediaTek added this wrapper in
 * ver02 purely so vow_scp.c (a separate translation unit) could reach the
 * same flag. Implementation ported verbatim from the vendor platform
 * tree's ver02/vow.c.
 */
