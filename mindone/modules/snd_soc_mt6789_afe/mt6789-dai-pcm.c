				 ...
				 struct snd_soc_dai *dai)
{
	...
	dev_info(afe->dev, "%s(), id %d, stream %d, rate %d, rate_reg %d, widget active p %d, c %d\n",
		 ...
		 rate,
		 rate_reg,
		 snd_soc_dai_get_widget(dai, SNDRV_PCM_STREAM_PLAYBACK)->active,
		 snd_soc_dai_get_widget(dai, SNDRV_PCM_STREAM_CAPTURE)->active);

	if (snd_soc_dai_get_widget(dai, SNDRV_PCM_STREAM_PLAYBACK)->active || snd_soc_dai_get_widget(dai, SNDRV_PCM_STREAM_CAPTURE)->active)
		return 0;
