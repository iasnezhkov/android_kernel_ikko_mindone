 ...
 *
 * This file is the missing ID table, merged from two independent 6.1
 * GPL vendor drops that both trace back to the same MTK ATF header:
 *   - NothingOSS/android_kernel_device_modules_6.1_nothing_mt6878
 *     (branch mt6878/Tetris/u), include/linux/soc/mediatek/mtk_sip_svc.h
 *   - a second independent 6.1/5.10-era vendor tree's copy of the same path,
 *     independently confirming the same ID numbering.
 * Merged as the union of both (the second tree adds EMIMPU_READ/WRITE/SET,
 * TMEM, DAPC_MMUP_GET beyond the Nothing set).
 *
