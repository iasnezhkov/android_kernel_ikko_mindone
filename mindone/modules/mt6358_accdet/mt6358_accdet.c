...
#include <sound/soc/codecs/mt6358-accdet.h>
#include "mt6358.h"
...
static enum alarmtimer_restart endoscope_alarm_timer_func(struct alarm *alarm, ktime_t now)
{
	...
	if (dev->eoc_irq_sta)
	{
		...
		pinctrl_select_state(eoc_dev->eoc_pinctrl, eoc_dev->eoc_switch_low);  //���ȳ��� ����
...
static irqreturn_t endoscope_int_handler(int irq, void *dev_id)
{
	...
	if(cur_ptt_eint_state == 1 )/* vendor comment, original encoding corrupted */
	{
		irq_set_irq_type(eoc_dev->eoc_irq_ptt, IRQF_TRIGGER_NONE);
		cur_ptt_eint_state = 0;
		disable_irq_nosync(eoc_dev->eoc_irq_ptt);   /* disable button IRQ */
	...
	if((accdet_val  < 1200)&& (accdet_val !=0))   //accdet_val  == 800mv  1200
	{
		...
		if(cur_ptt_eint_state == 0)
		{
		  ...
		  if (ret) {
			...
			enable_irq(eoc_dev->eoc_irq_ptt);   /* enable button IRQ */
		}
	}
	else
	{
...
static int endoscope_probe(struct platform_device *pdev)
{
	...
	disable_irq(eoc_dev->eoc_irq_ptt);   /* disable button IRQ */
	if (ret) {
...
static void endoscope_remove(struct platform_device *dev)	
{
...
static struct platform_driver accdet_driver = {
	...
	.remove_new = mt6358_accdet_remove,
	.driver = {
