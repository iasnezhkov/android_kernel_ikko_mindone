				 ...
				 struct task_struct *p)
{
	...
	if (!uclamp_is_used() &&
	    type == MTK_FREQUENCY_UTIL && rt_rq_is_runnable(&rq->rt)) {
		return max;
	}
...
static unsigned long sugov_get_util(struct sugov_cpu *sg_cpu)
{
	...
	return mtk_cpu_util(sg_cpu->cpu, util, max, MTK_FREQUENCY_UTIL, NULL);
}
