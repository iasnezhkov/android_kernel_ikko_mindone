...
/* k6 bring-up: copied from the MediaTek platform sugov cpufreq.h under a distinct name -- this build-dir already has its own "cpufreq.h" (the generic linux/cpufreq.h, needed for struct cpufreq_policy), and the vendor sugov-specific header of the same basename was silently lost to that quote-include collision (k6-schedh-port). Different include guard (__CPUFREQ_H__ vs _LINUX_CPUFREQ_H), no clash. */
/*
