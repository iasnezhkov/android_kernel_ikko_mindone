...
/* V4L2_CTRL_CLASS_MPEG is only aliased to V4L2_CTRL_CLASS_CODEC under #ifndef __KERNEL__ (userspace-only back-compat, real uapi/linux/v4l2-controls.h) -- kernel code (this file) must use the real modern name directly. Same numeric value either way, this only affects visibility in kernel builds. */
