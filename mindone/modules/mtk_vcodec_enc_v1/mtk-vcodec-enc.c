			    ...
			    struct v4l2_buffer *buf)
{
	...
	if (buf->flags & V4L2_BUF_FLAG_HAS_META &&
		...
		void *meta_va = NULL;
		/* dma_buf_vmap/vunmap moved to the struct iosys_map * API
		 * upstream (real, same fix as elsewhere in this repo). */
		...
		if (!dma_buf_vmap(mtkbuf->frm_buf.metabuffer_dma, &meta_map))
			meta_va = meta_map.is_iomem ? NULL : meta_map.vaddr;
	...
	struct dma_buf *dbuf, unsigned long size)
{
	...
	enum dma_data_direction dma_dir = vb->vb2_queue->dma_dir;
