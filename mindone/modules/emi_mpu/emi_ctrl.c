...
MINDONE_DEFINE_SEMAPHORE(emi_bwl_sem);
...
/* MINDONE-EMI-NODES: fallback path to get a region by DT node. The bind
 * node carries only one of three regions; the missing ones live in separate
 * nodes. Returns NULL if the node is not found -- the caller decides whether
 * that counts as a failure.
 */
static void __iomem *mindone_emi_map(const char *compat)
{
	struct device_node *n = of_find_compatible_node(NULL, NULL, compat);
	void __iomem *p;

	if (!n) {
		pr_info("[EMI] MINDONE: node %s not found\n", compat);
		return NULL;
	}
	p = of_iomap(n, 0);
	of_node_put(n);
	pr_info("[EMI] MINDONE: %s mapped at %p\n", compat, p);
	return p;
}
...
static int emi_probe(struct platform_device *pdev)
{
	...
	if (res)
		...
		CHA_EMI_BASE = mindone_emi_map("mediatek,common-emichn");
	if (IS_ERR_OR_NULL(CHA_EMI_BASE)) {
	...
	if (res)
		...
		EMI_MPU_BASE = mindone_emi_map("mediatek,emi_mpu");
	if (IS_ERR_OR_NULL(EMI_MPU_BASE)) {
...
static void emi_remove(struct platform_device *dev)
{
	return;
}
