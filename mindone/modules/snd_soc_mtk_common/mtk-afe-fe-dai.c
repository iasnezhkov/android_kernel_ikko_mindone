		       ...
		       struct snd_soc_dai *dai)
{
	...
	int memif_num = MINDONE_RTD_TO_CPU(rtd, 0)->id;
...
int mtk_afe_suspend(struct snd_soc_component *component)
{
	...
	afe->suspended = true;
	/* D-5 (AUDIO-HANG): go through pm_runtime core instead of
	 * calling afe->runtime_suspend() as a raw function pointer, so this
	 * path is serialized against a concurrent pm_runtime_get_sync() ->
	 * dev_pm_ops.runtime_resume() on another thread (F2718 race) by the
	 * same dev->power.lock / RPM_RESUMING-RPM_SUSPENDING state machine.
	 */
...
int mtk_afe_resume(struct snd_soc_component *component)
{
	...
	/* D-5-fix: do NOT check pm_runtime_status_suspended() here.
	 * pm_runtime_force_suspend() in mtk_afe_suspend() ITSELF sets
	 * runtime_status = RPM_SUSPENDED, so the previous condition would ALWAYS
	 * fire and pm_runtime_force_resume() would never be called: runtime PM
	 * would stay disabled, registers would not be restored, sound would not
	 * wake up. The afe->suspended flag already unambiguously marks that WE
	 * did the suspend.
	 */
	if (!afe->suspended)
		return 0;
