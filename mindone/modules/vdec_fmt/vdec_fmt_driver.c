 ...
 * hits anywhere in kernel6/common61). Unlike the camera_* modules, this
 * ioctl's native handler (fmt_gce_cmd_flush(), below) is monolithic: it
 * takes the raw unsigned long ioctl arg and does its own single
 * copy_from_user() of the whole struct gce_cmdq_obj envelope, rather than
 * a thin ioctl-dispatcher passing an already-copied struct into a separate
 * business function. So instead of a synthetic native-layout struct in
 * user space (compat_alloc_user_space()) plus a recursive
 * filp->f_op->unlocked_ioctl() call, the fix splits fmt_gce_cmd_flush()
 * into this business function (unchanged body, taking an
 * already-populated kernel-space struct gce_cmdq_obj by value) and a thin
 * wrapper (below) that keeps doing the exact same copy_from_user() against
 * the real native-caller pointer as before. The compat handler further
 * down populates its own kernel-local struct gce_cmdq_obj field-by-field
 * (compat_ptr() for the "taskid" out-pointer, plain get_user() for the
 * scalars -- cmds_user_ptr is copied as a raw u64 value with no
 * compat_ptr() translation, matching the original compat_get_gce_cmdq_obj_data()
 * helper exactly) and calls this business function directly.
 *
 * The original fmt->fmt_error precondition check ran before the
 * copy_from_user() in fmt_gce_cmd_flush(), textually first but with no
 * functional dependency on it -- every one of that check's, the
 * IS_ERR_OR_NULL(arg) check's, and the copy_from_user() failure path's
 * return values is the same -EINVAL, so moving the fmt_error check to the
 * top of this shared business function (instead of leaving it stranded in
 * the native-only wrapper) does not change observable native-path
 * behavior, and gives the compat path the same guard the native path always
 * had.
 */
static int fmt_gce_cmd_flush_business(struct gce_cmdq_obj buff)
{
	int i, ret;
	int taskid = -1;
...
static int fmt_gce_cmd_flush(unsigned long arg)
{
	...
	return fmt_gce_cmd_flush_business(buff);
}
				...
				unsigned long arg)
{
	...
	case COMPAT_FMT_GCE_SET_CMD_FLUSH:
	{
		...
		buff.cmds_user_ptr = u64t;
		err |= get_user(u32t, &data32->identifier);
		buff.identifier = u32t;
		err |= get_user(u32t, &data32->secure);
		buff.secure = u32t;
		err |= get_user(ptrt, &data32->taskid);
		buff.taskid = (typeof(buff.taskid))compat_ptr(ptrt);
		err |= get_user(u32t, &data32->pmqos_param.tv_sec);
		buff.pmqos_param.tv_sec = u32t;
		err |= get_user(u32t, &data32->pmqos_param.tv_usec);
		buff.pmqos_param.tv_usec = u32t;
		err |= get_user(u32t, &data32->pmqos_param.pixel_size);
		buff.pmqos_param.pixel_size = u32t;
		err |= get_user(u32t, &data32->pmqos_param.rdma_datasize);
		buff.pmqos_param.rdma_datasize = u32t;
		err |= get_user(u32t, &data32->pmqos_param.wdma_datasize);
		buff.pmqos_param.wdma_datasize = u32t;
		if (err != 0)
			return err;

		/*
		 * fmt_gce_cmd_flush_business() does its own copy_from_user()
		 * of the cmds_user_ptr payload and copy_to_user() of the
		 * assigned task id back through buff.taskid (translated
		 * above via compat_ptr()) -- both against real __user
		 * pointers, so there is nothing further to copy back into
		 * data32 here (matching the original compat_get_gce_cmdq_obj_data(),
		 * which never had a corresponding "put" helper for this
		 * ioctl at all).
		 */
		return fmt_gce_cmd_flush_business(buff);
	}
...
static int vdec_fmt_probe(struct platform_device *pdev)
{
	...
	fmt->fmt_class = MINDONE_CLASS_CREATE(fmt->fmtname);
