 ...
 * maintain independently of bms. */
	...
	case POWER_SUPPLY_PROP_CURRENT_AVG:
	{
		...
		cur_ret = gauge_get_property(GAUGE_PROP_BATTERY_CURRENT,
			&ibat_now);
		if (cur_ret < 0)
			ibat_now = gm->ibat; /* gauge unavailable: last known value */
		else
			gm->ibat = ibat_now; /* keep other consumers' cache fresh */

		val->intval = ibat_now * 100;
		break;
	}
