				...
				int brightness)
{
	...
	brightness = brightness * 87 / 100;//prize add by dengzhiyuan 20230801
...
static int mtk_leds_brightness_resend(const char *name)
{
	...
	index = get_desp_index((char *)name);
	if (index < 0)
		return -ENODEV;
	led_dat = container_of(leds_info->leds[index], struct mt_led_data, desp);
	mutex_lock(&led_dat->led_access);
	level = led_dat->hw_brightness;
	if (level > 0)
		mtk_set_hw_brightness(led_dat, level);
	mutex_unlock(&led_dat->led_access);
...
static int mindone_bl_resend_notify(struct notifier_block *nb, unsigned long event, void *data)
{
	if (event != MTK_DISP_EVENT_LK_PANEL_ON)
		return NOTIFY_DONE;
	mtk_leds_brightness_resend("lcd-backlight");
	return NOTIFY_OK;
}

static struct notifier_block mindone_bl_resend_nb = {
	.notifier_call = mindone_bl_resend_notify,
};
