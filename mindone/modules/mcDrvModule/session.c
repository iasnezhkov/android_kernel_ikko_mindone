...
/*
 * get_task_exe_file lost its EXPORT_SYMBOL in this kernel tree (it is a
 * kernel-internal helper now, not available to external modules). Its
 * implementation in kernel/fork.c is built only from primitives that ARE
 * exported/public: get_task_mm() (EXPORT_SYMBOL_GPL) already does the exact
 * same task_lock()+PF_KTHREAD+mmget() dance as get_task_exe_file uses
 * internally, so re-deriving exe_file from mm here (rcu_dereference of
 * mm->exe_file + get_file_rcu, same as upstream get_mm_exe_file) reproduces
 * identical behavior without needing the removed export. Not a stub -- same
 * real logic, just inlined locally.
 */
static struct file *mc_get_task_exe_file(struct task_struct *task)
{
			      ...
			      const void *data, unsigned int data_len)
{
	...
	exe_file = mc_get_task_exe_file(task);
	if (!exe_file) {
		ret = -ENOENT;
		goto end;
	}
