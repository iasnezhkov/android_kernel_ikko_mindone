...
/*
 * ION (linux/ion.h, ion_priv.h) was removed from upstream Linux ~5.11+ and
 * replaced by the generic DMA-BUF heaps framework (CONFIG_DMABUF_HEAPS).
 * Neither header exists in this kernel tree -- do not re-add them. See the
 * tee_mmu_buffer() cacheability check below for the corresponding logic
 * change: DMA-BUF heaps does not expose a static per-buffer cached/uncached
 * flag the way ION did (no exporter in this tree implements the optional
 * dma_buf_ops.get_flags hook), so that concept can no longer be queried from
 * a bare struct dma_buf pointer here.
 */
...
void tee_mmu_buffer(struct tee_mmu *mmu, struct mcp_buffer_map *map)
{
	...
	if (mmu->dma_buf) {
		/*
		 * Under ION (pre-5.11), a buffer could be allocated
		 * non-cacheable and this was queried via the private
		 * struct ion_buffer.flags & ION_FLAG_CACHED bit. ION is gone;
		 * DMA-BUF heaps has no equivalent static flag on the generic
		 * struct dma_buf (dma_buf_get_flags() exists but no heap
		 * exporter in this tree implements .get_flags, so it always
		 * fails). Under DMA-BUF heaps, generic heap allocations
		 * (system/CMA/page-pool heaps) are CPU-cacheable by
		 * convention -- coherency is managed dynamically by the
		 * importer via dma_buf_begin_cpu_access()/end_cpu_access(),
		 * not a static per-buffer bit. map->type already defaults to
		 * cacheable (WSM_UNCACHED not set), so the correct behavior
		 * for a dma_buf-backed region here is to leave it as-is
		 * rather than force WSM_UNCACHED based on a flag that no
		 * longer exists in this allocator model.
		 */
		mc_dev_devel("dma_buf backed WSM: treated as cacheable (DMA-BUF heaps, no static ION-style flag)");
	}
#endif
	map->mmu = mmu;
}
