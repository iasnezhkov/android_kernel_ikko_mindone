...
unsigned char TneGvc(void)
{
	...
	/* 64夞偺暯嬒抣應掕     GYRMON2(0x1111) <- GYADZ(0x14CA) */
...
void SetZsp(unsigned char UcZoomStepDat)
{
	...
	if (UcZoomStepDat > (ZOOMTBL - 1))
		UcZoomStepDat = (ZOOMTBL - 1); /* 忋尷傪ZOOMTBL-1偵愝掕偡傞 */
	...
	while (UcClkJdg == UNDR_MEAS) {
		...
		if (!(UcMeasFlg & RSELFX)) {
			if (UcMeasFlg & RSEL1ST)
				...
				UcMeasFlg |= RSEL1ST;
			ScTblRate_Now = ScRselRate[UcOscrsel]; /* 崱偺Rate */
...
void SetGcf(unsigned char UcSetNum)
{
	...
	if (UcSetNum > (COEFTBL - 1))
		UcSetNum = (COEFTBL - 1); /* 忋尷傪COEFTBL-1偵愝掕偡傞 */
