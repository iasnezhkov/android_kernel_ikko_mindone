...
int scmi_tinysys_event_notify(u32 feature_id, u32 notify_enable)
{
	...
	if(notify_enable) {
		ret = handle->notify_ops->event_notifier_register(handle, SCMI_PROTOCOL_TINYSYS,
			SCMI_EVENT_TINYSYS_NOTIFIER, &f_id, &tinysys_nb);	//fix NULL
		if(ret)
			pr_notice("scmi register_event_notifier f_id:%d ret:%d\n",f_id, ret);

	} else {
		ret = handle->notify_ops->event_notifier_unregister(handle, SCMI_PROTOCOL_TINYSYS,
			SCMI_EVENT_TINYSYS_NOTIFIER, &f_id, &tinysys_nb);	//fix NULL
		if(ret)
