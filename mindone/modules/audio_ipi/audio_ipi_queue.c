	...
	struct ipi_msg_t *p_ipi_msg)
{
	...
	if (is_audio_task_dsp_ready(p_ipi_msg->task_scene) == false) {
		/* MT6789 has no audio DSP; the HAL keeps trying (~7/s). Two lines a minute are enough. */
		static DEFINE_RATELIMIT_STATE(rs_not_ready, 60 * HZ, 2);

		if (__ratelimit(&rs_not_ready))
			pr_info("dsp not ready!! return (scene %d, rate-limited)", p_ipi_msg->task_scene);
		return -1;
	}
