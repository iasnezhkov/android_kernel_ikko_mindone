...
bool is_audio_dsp_ready(const uint32_t dsp_id)
{
	...
	if (is_audio_use_adsp(dsp_id)) {
		...
		/* Once per dsp_id: the HAL asks for the (absent) audio DSP ~7/s. */
		static unsigned long warned;

		if (dsp_id < BITS_PER_LONG && !test_and_set_bit(dsp_id, &warned))
			pr_notice("dsp_id %u not support!! (reported once)", dsp_id);
	}
