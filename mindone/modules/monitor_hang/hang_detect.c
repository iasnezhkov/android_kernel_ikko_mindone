...
void show_thread_info(struct task_struct *p, bool dump_bt)
{
	...
	/* MINDONE-HANG-TASKCPU: task_struct.cpu was folded into thread_info
	 * and is no longer a direct field; task_cpu() is the stable exported
	 * accessor (include/linux/sched.h) that reads it the same way.
	 */
...
#ifdef CONFIG_STACKTRACE
	/* MINDONE-HANG-TASKSTATE: task_struct.state was renamed to __state
	 * (accessed with READ_ONCE, see include/linux/sched.h task_is_running()
	 * and friends); read it the same way here.
	 */
...
static int dump_native_maps(pid_t pid, struct task_struct *current_task)
{
	...
	/* MINDONE-HANG-MAPLE: mm->mmap (singly linked vma list) and
	 * vma->vm_next were removed when the mm subsystem switched to a
	 * maple tree; walk it with the exported VMA_ITERATOR()/for_each_vma()
	 * API (linux/mm.h, linux/mm_types.h) instead of following vm_next.
	 */
	{
	VMA_ITERATOR(vmi, current_task->mm, 0);
	...
	struct task_struct *current_task)
{
	...
	if (compat_user_mode(user_ret)) {	/* K64_U32 for check reg */
		...
		/* MINDONE-HANG-MAPLE: find_vma() (exported, linux/mm.h) returns
		 * the vma whose vm_end is nearest above userstack_start in the
		 * maple tree, replacing the removed mm->mmap/vm_next chain
		 * walk; containment is still checked explicitly below exactly
		 * as the original loop did.
		 */
		vma = find_vma(current_task->mm, userstack_start);
		...
		/* MINDONE-HANG-MAPLE: see the K64+U32 branch above for why
		 * find_vma() replaces the vm_next chain walk here.
		 */
		vma = find_vma(current_task->mm, userstack_start);
		if (vma && vma->vm_start <= userstack_start &&
				vma->vm_end >= userstack_start)
