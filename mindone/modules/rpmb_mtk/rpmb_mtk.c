...
/* provided by ufs_mtk_rpmb.ko (our own module, no shared header exists upstream) */
...
/* Mirrors uapi struct mmc_ioc_cmd (include/uapi/linux/mmc/ioctl.h) field-for-field - only
 * the fields we actually read/use are named, the rest exist purely to keep sizeof() at the
 * real 72 bytes so MTK_RPMB_MMC_IOC_CMD below decodes to the exact wire value teed sends.
 */
struct mtk_rpmb_mmc_ioc_cmd {
	int write_flag;
	int is_acmd;
	u32 opcode;
	u32 arg;
...
#define MTK_RPMB_MMC_BLOCK_MAJOR   179   /* MMC_BLOCK_MAJOR, include/linux/mmc/blkdev.h */
#define MTK_RPMB_MMC_SEND_EXT_CSD  8     /* MMC_SEND_EXT_CSD opcode */
#define MTK_RPMB_EXT_CSD_RPMB_SIZE_MULT  168
#define MTK_RPMB_EXT_CSD_REL_WR_SEC_C    222
#define MTK_RPMB_MMC_IOC_CMD \
	_IOWR(MTK_RPMB_MMC_BLOCK_MAJOR, 0, struct mtk_rpmb_mmc_ioc_cmd)

extern void ufs_mtk_rpmb_get_geometry(u16 *out_capacity, u16 *out_reliable_wr_count);
extern int ufs_mtk_rpmb_xfer(u8 *req, unsigned int req_len, u8 *resp, unsigned int resp_len);

static long rpmb_ioctl_get_reliable_wr_count(unsigned long arg)
{
	struct rpmb_reliable_wr_info info;
	u16 rw_count = 0;

	ufs_mtk_rpmb_get_geometry(NULL, &rw_count);

	memset(&info, 0, sizeof(info));
	info.rel_wr_sec_c = rw_count;
...
static long rpmb_ioctl_mmc_send_ext_csd(unsigned long arg)
{
	struct mtk_rpmb_mmc_ioc_cmd mcmd;
	u8 *ext_csd;
	u16 capacity = 0, rw_count = 0;
	long ret = 0;

	if (copy_from_user(&mcmd, (void __user *)arg, sizeof(mcmd)))
		return -EFAULT;

	/* only emulate the one call shape teed's read_extcsd() actually sends */
	if (mcmd.opcode != MTK_RPMB_MMC_SEND_EXT_CSD || mcmd.blocks != 1 ||
	    mcmd.blksz == 0 || mcmd.blksz > 4096 || !mcmd.data_ptr)
		return -ENOTTY;

	ext_csd = kzalloc(mcmd.blksz, GFP_KERNEL);
	if (!ext_csd)
		return -ENOMEM;

	ufs_mtk_rpmb_get_geometry(&capacity, &rw_count);

	if (mcmd.blksz > MTK_RPMB_EXT_CSD_RPMB_SIZE_MULT)
		ext_csd[MTK_RPMB_EXT_CSD_RPMB_SIZE_MULT] = (u8)capacity;
	if (mcmd.blksz > MTK_RPMB_EXT_CSD_REL_WR_SEC_C)
		ext_csd[MTK_RPMB_EXT_CSD_REL_WR_SEC_C] = (u8)rw_count;

	if (copy_to_user((void __user *)(uintptr_t)mcmd.data_ptr, ext_csd, mcmd.blksz))
		ret = -EFAULT;
...
static long rpmb_ioctl_tk_frames(unsigned int cmd, unsigned long arg)
{
	struct mindone_tk_rpmb_arg a;
	u8 *frames;
	u32 n;
	size_t sz;
	u16 type;
	...
	if (copy_from_user(&a, (void __user *)arg, sizeof(a)))
		return -EFAULT;
	n = a.blk_cnt;
	if (n == 0 || n > MINDONE_TK_RPMB_MAX_BLKS || !a.buf)
		return -EINVAL;
	if (cmd != MINDONE_TK_RPMB_IOCTL_READ_DATA && n != 1)
		return -EINVAL;
	sz = (size_t)n * RPMB_SZ_FRAME;
	frames = kzalloc(sz, GFP_KERNEL);
	if (!frames)
		return -ENOMEM;
	/* the first frame is the request from the TEE */
	if (copy_from_user(frames, (void __user *)(uintptr_t)a.buf, RPMB_SZ_FRAME)) {
		kfree(frames);
		return -EFAULT;
	}
	type = be16_to_cpu(((struct rpmb_frame *)frames)->req_resp);

	/*
	 * Transport is ufs_mtk_rpmb_xfer() (SECURITY PROTOCOL OUT/IN, with a separate
	 * "result read" for writes); the vendor's original rpmb_req_*_ufs() are unusable here -- their
	 * rpmb_cmd_req() without CONFIG_RPMB is a stub (B16c: response = request, TEE
	 * "Unexpected msg_type 0x0002 != 0x0200").
	 * Request and response share one buffer: the request goes out before the response
	 * is read.
	 */
	switch (cmd) {
	case MINDONE_TK_RPMB_IOCTL_GET_WC:
		if (type != RPMB_GET_WRITE_COUNTER)
			MSG(ERR, "%s: ioctl 12 with req_resp 0x%x\n", __func__, type);
		ret = ufs_mtk_rpmb_xfer(frames, RPMB_SZ_FRAME, frames, RPMB_SZ_FRAME);
		break;
	case MINDONE_TK_RPMB_IOCTL_READ_DATA:
		if (type != RPMB_READ_DATA)
			MSG(ERR, "%s: ioctl 11 with req_resp 0x%x\n", __func__, type);
		/* UFS requires block_count in a read request; the TEE (as for eMMC) leaves it 0 */
		((struct rpmb_frame *)frames)->block_count = cpu_to_be16((u16)n);
		ret = ufs_mtk_rpmb_xfer(frames, RPMB_SZ_FRAME, frames, sz);
		break;
	case MINDONE_TK_RPMB_IOCTL_WRITE_DATA:
		if (type != RPMB_WRITE_DATA)
			MSG(ERR, "%s: ioctl 10 with req_resp 0x%x\n", __func__, type);
		ret = ufs_mtk_rpmb_xfer(frames, RPMB_SZ_FRAME, frames, RPMB_SZ_FRAME);
		break;
	default:
		kfree(frames);
	...
	MSG(INFO, "%s: ioctl %u type 0x%x blks %u -> ret %d result 0x%x\n", __func__, cmd, type, n,
	    ret, be16_to_cpu(((struct rpmb_frame *)frames)->result));
	if (copy_to_user((void __user *)(uintptr_t)a.buf, frames, sz)) {
		kfree(frames);
		return -EFAULT;
	}
	kfree(frames);
...
static long rpmb_ioctl_ufs(struct file *pfile, unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == MTK_RPMB_MMC_IOC_CMD)
		return rpmb_ioctl_mmc_send_ext_csd(arg);
	if (cmd == MINDONE_TK_RPMB_IOCTL_WRITE_DATA || cmd == MINDONE_TK_RPMB_IOCTL_READ_DATA ||
	    cmd == MINDONE_TK_RPMB_IOCTL_GET_WC)
		return rpmb_ioctl_tk_frames(cmd, arg);
