		...
		unsigned int ipi_chan_count)
{
	...
	for (index = 0; index < ipi_chan_count; index++) {
		...
		pr_debug("MINDONE-IPI: idx=%d pre-ept rpdev=%p ops=%p mchan=%p\n",
			index, &mtk_rpdev->rpdev, mtk_rpdev->rpdev.ops,
			mtk_rpchan);
		ipi_chan_table[index].ept =
			rpmsg_create_ept(&(mtk_rpdev->rpdev),
					NULL, mtk_rpchan, mtk_rpchan->info);
		pr_debug("MINDONE-IPI: idx=%d post-ept ept=%p\n",
			index, ipi_chan_table[index].ept);
