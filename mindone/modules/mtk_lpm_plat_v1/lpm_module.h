...
/*
 * The GKI/ACK-tree copy of linux/soc/mediatek/mtk_sip_svc.h (picked up
 * ahead of the real vendor one because it lives on the kernel include
 * search path) lacks the MTK-vendor-private SMC ids -- including
 * MTK_SIP_MTK_LPM_CONTROL used below. Value copied verbatim (not
 * invented) from a vendor platform tree's linux/soc/mediatek/
 * mtk_sip_svc.h (MTK_SIP_SMC_CMD(0x507) expansion), only defined here to
 * avoid colliding with/shadowing the real system header for anything
 * else that might include it.
 */
#ifndef MTK_SIP_MTK_LPM_CONTROL
#define MTK_SIP_MTK_LPM_CONTROL \
	ARM_SMCCC_CALL_VAL(ARM_SMCCC_FAST_CALL, ARM_SMCCC_SMC_64, \
			   ARM_SMCCC_OWNER_SIP, 0x507)
#endif
