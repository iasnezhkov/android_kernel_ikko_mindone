...
static int mtk_lpm_monitor_thread(void *data)
{
	...
	/* do_exit(0) not exported in this kernel; returning 0 from a kthread
	 * entry fn is the documented equivalent -- kernel/kthread.c kthread()
	 * calls do_exit(threadfn_retval) right after the thread fn returns,
	 * so "return 0;" here reaches the identical real do_exit(0). */
	return 0;
}
			...
			unsigned long pm_event, void *unused)
{
	...
	case PM_SUSPEND_PREPARE:
		...
		cpus_read_lock(); /* renamed from get_online_cpus(), same real upstream API, ~5.x rename */
		for_each_online_cpu(cpu) {
			...
			if (!IS_ERR(mtk_lpm_ac[cpu].ts)) {
				...
				spin_unlock(&lpm_abort_locker);
				cpus_read_unlock(); /* renamed from put_online_cpus() */
				return NOTIFY_BAD;
			}
