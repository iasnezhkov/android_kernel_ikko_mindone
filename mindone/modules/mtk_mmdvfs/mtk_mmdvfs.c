...
static int mmdvfs_probe(struct platform_device *pdev)
{
	...
	of_property_for_each_string(dev->of_node, "mediatek,support_hopping",
		...
		num_hopping_rate = 0;
		hopping_rate_cnt = of_property_count_u32_elems(dev->of_node, prop_name);
		for (i = 0; i < hopping_rate_cnt; i++) {
			if (num_hopping_rate >= MAX_OPP_NUM) {
				pr_notice("Too many items in %s\n", prop_name);
				return -EINVAL;
			}
			of_property_read_u32_index(dev->of_node, prop_name, i, &hopping_rate);
