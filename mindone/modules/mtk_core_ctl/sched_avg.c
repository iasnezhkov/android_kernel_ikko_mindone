/* MINDONE_CAP_COMPAT 12.09: capacity_orig_of() was removed in 6.9 */
#include <linux/version.h>
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 9, 0)
#define capacity_orig_of(cpu) arch_scale_cpu_capacity(cpu)
#endif
#include <linux/version.h>
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 9, 0)
		/* MINDONE 12.09: since 6.9 util_est is just an unsigned int (no .enqueued);
		 * we drop the UTIL_AVG_UNCHANGED flag in the low bit. */
		util = max_t(unsigned long, util,
			READ_ONCE(cfs_rq->avg.util_est) & ~UTIL_AVG_UNCHANGED);
#else
		util = max_t(unsigned long, util,
...
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 9, 0)
	/* MINDONE 12.09: since 6.9 util_est is an unsigned int without ewma */
...
static void over_thresh_chg_notify(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		raw_spin_lock_irqsave(rq_lockp(cpu_rq(cpu)), flags);
		...
		if (!cpu_online(cpu)) {
			...
			raw_spin_unlock_irqrestore(rq_lockp(cpu_rq(cpu)), flags);
			continue;
		}
...
/* task_of() now comes from kernel/sched/sched.h (k6 bring-up: common61 -I
 * redirect, k6-schedh-port) -- this module's own copy duplicated it
 * and now conflicts with the real one. */
...
int get_over_thres_stats(char *buf, int buf_size)
{
	...
	for_each_possible_cpu(cpu) {
		...
		len += snprintf(buf+len, buf_size-len,
			...
			cpu, arch_scale_cpu_capacity(cpu), /* MINDONE: cpu_capacity_orig was removed in 6.9 */
			cpu_over_thres->dn_thres,
			cpu_over_thres->up_thres);
	}
...
static int init_attribs(void)
{
	...
	{
		struct device *cpu_root = bus_get_dev_root(&cpu_subsys); /* MINDONE: dev_root was removed in 6.7 */

		if (cpu_root) {
			kobj = kobject_create_and_add("sched-avg", &cpu_root->kobj);
			put_device(cpu_root);
		}
	}
#else
