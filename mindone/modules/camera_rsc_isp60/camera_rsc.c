 ...
 ******************************************************************************/
/*
 * compat_alloc_user_space() was removed upstream (kernel 5.18+; confirmed 0
 * hits anywhere in kernel6/common61). Same shape as camera_fdvt_isp51's
 * fix: RSC_ENQUE_REQ/RSC_DEQUE_REQ transfer a variable-length
 * struct RSC_Config[] array through a pointer field embedded in
 * RSC_Request, using copy_from_user()/copy_to_user() directly against that
 * pointer, so there's no synthetic native struct or kmalloc staging buffer
 * needed. Splitting the ring-buffer business logic out of the switch/case
 * lets the compat handler below call it directly with a
 * compat_ptr()-translated pointer, instead of recursing through
 * filp->f_op->unlocked_ioctl() via compat_alloc_user_space().
 *
 * fdvt_enque_req_business() kept its two failing paths as plain returns
 * because FDVT's mutex_lock() only started after those checks. Here
 * gRscMutex is held across the *entire* original case body (mutex_lock()
 * precedes even the outer copy_from_user()), so it stays owned by the
 * caller (both the native wrapper below and the compat handler take/drop
 * it around their call to this function) -- the two early-failure
 * "mutex_unlock(); goto EXIT;" pairs from the original case body are
 * therefore just plain early returns here, since business() never touches
 * the mutex itself. The rest of the body is unmodified from the original
 * case.
 */
static signed int rsc_enque_req_business(struct RSC_Request rsc_RscReq,
...
/*
 * DEQUE_REQ mirrors camera_fdvt_isp51's fdvt_deque_req_business(): body is
 * the original case's inner block verbatim (goto EXIT -> goto
 * exit_business, local label instead of the shared error logger at the end
 * of RSC_ioctl), operating on a local copy of the RSC_Request struct that
 * gets written back to *preq before returning. m_pRscConfig is never
 * mutated by this body, so the caller can safely re-check *preq's
 * m_pRscConfig after the call to decide whether to write the struct back
 * to the original caller -- exactly like the original single-pass code did.
 */
static signed int rsc_deque_req_business(struct RSC_Request *preq)
{
	signed int Ret = 0;
	int dequeNum;
	unsigned long flags;
	struct RSC_Request rsc_RscReq = *preq;
	...
	case RSC_ENQUE_REQ:
		{
			...
			if (copy_from_user(&rsc_RscReq, (void *)Param,
				...
				Ret = rsc_enque_req_business(rsc_RscReq, pUserInfo);
			} else {
				LOG_ERR(
	...
	case RSC_DEQUE_REQ:
		{
			if (copy_from_user(&rsc_RscReq, (void *)Param,
					sizeof(struct RSC_Request)) == 0) {
				Ret = rsc_deque_req_business(&rsc_RscReq);

				/*
				 * rsc_deque_req_business() never mutates
				 * m_pRscConfig, so re-checking it here
				 * reproduces the original single-pass code's
				 * decision of whether to write the struct
				 * back to the caller.
				 */
				if (rsc_RscReq.m_pRscConfig != NULL) {
					if (copy_to_user
							...
							unsigned long arg)
{
	...
	case COMPAT_RSC_READ_REGISTER:
		{
			...
			err |= get_user(RegIo.Count, &data32->Count);
			if (err)
				return -EFAULT;
			RegIo.pData = (struct RSC_REG_STRUCT *)compat_ptr(uptr);

			/*
			 * RSC_ReadReg() already does its own
			 * get_user()/put_user() against RegIo.pData as a
			 * real __user pointer (translated above via
			 * compat_ptr()) and validates RegIo.Count internally
			 * -- nothing to copy back into data32 (Count is
			 * never mutated).
			 */
			ret = RSC_ReadReg(&RegIo);
			return ret;
	...
	case COMPAT_RSC_WRITE_REGISTER:
		{
			/*
			 * Pre-existing vendor gap, not something introduced
			 * by this fix: RSC_ioctl's native switch has no
			 * "case RSC_WRITE_REGISTER" at all (no RSC_WriteReg()
			 * function exists in this driver). The original
			 * compat handler recursed into
			 * filp->f_op->unlocked_ioctl(filp, RSC_WRITE_REGISTER, ...),
			 * which always fell through to RSC_ioctl's default:
			 * case and returned -EPERM. Preserve that exact
			 * observable behavior directly, since there is no
			 * native business logic to call here.
			 */
			return -EPERM;
		}
	case COMPAT_RSC_ENQUE_REQ:
		{
			...
			if (filp->private_data == NULL)
				return -EFAULT;
			pUserInfo =
				(struct RSC_USER_INFO_STRUCT *)filp->private_data;

			data32 = compat_ptr(arg);
			if (data32 == NULL)
				return -EFAULT;

			err = get_user(rsc_RscReq.m_ReqNum, &data32->m_ReqNum);
			err |= get_user(uptr, &data32->m_pRscConfig);
			if (err)
				return -EFAULT;
			rsc_RscReq.m_pRscConfig =
				(struct RSC_Config *)compat_ptr(uptr);
	...
	case COMPAT_RSC_DEQUE_REQ:
		{
			...
			if (rsc_RscReq.m_pRscConfig != NULL) {
				err = put_user(rsc_RscReq.m_ReqNum,
					       &data32->m_ReqNum);
				if (err) {
					LOG_ERR("RSC_DEQUE_REQ compat put_user failed\n");
					ret = -EFAULT;
				}
			}
			return ret;
			...
			unsigned long pm_event, void *unused)
{
	...
	case PM_SUSPEND_PREPARE: /*enter suspend*/
		...
		pr_debug(MyTag "%s:g_u4EnableClockCount(%d) g_SuspendCnt(%d).\n", __func__,
