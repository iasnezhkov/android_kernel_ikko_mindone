...
void mtk_tick_entry(void *data, struct rq *rq)
{
	...
	/* MINDONE-SCHED-THERMAL: arch_set_thermal_pressure() (MediaTek's own raw-
	 * pressure setter) does not exist on 6.1. It was renamed/redesigned
	 * upstream as arch_update_thermal_pressure() -> topology_update_thermal_
	 * pressure() (drivers/base/arch_topology.c, already EXPORT_SYMBOL_GPL'd
	 * there -- no kernel patch needed). That helper takes the capped
	 * frequency in kHz directly and performs internally the exact same
	 * freq -> capacity -> pressure conversion this code used to open-code
	 * by hand, so hand it freq_thermal instead of duplicating the math. */
...
void check_for_migration(struct task_struct *p)
{
	...
	if (rq->misfit_task_load) {
		...
		if (READ_ONCE(rq->curr->__state) != TASK_RUNNING ||
