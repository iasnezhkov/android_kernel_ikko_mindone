...
//gpio info
        ...
        if(dt->reset_gpio < 0 || dt->irq_gpio < 0){
            ...
            HYN_INFO("reset_gpio:%d irq_gpio:%d vdd_gpio:%d",dt->reset_gpio,dt->irq_gpio,dt->vdd_gpio);
        }
...
static int hyn_poweron(struct hyn_ts_data *ts_data)
{
    ...
    if(dt->vdd_gpio >= 0){
        ...
        if(gpio_direction_output(dt->vdd_gpio, 1)){
            HYN_ERROR("vdd gpio_direction_output failed, gpio=%d", dt->vdd_gpio);
        }
        else{
            HYN_INFO("vdd_gpio:%d driven high", dt->vdd_gpio);
        }
        mdelay(10);
    }
    else{
        HYN_INFO("vdd_gpio not config");
    }
                                          ...
                                          unsigned long val, void *v)
{
    ...
    if (!blank)
        return NOTIFY_DONE;
    if (val == MTK_DISP_EARLY_EVENT_BLANK && *blank == MTK_DISP_BLANK_POWERDOWN) {
        cancel_work_sync(&hyn_data->work_resume);
        hyn_suspend(hyn_data->dev);
        return NOTIFY_OK;
    }
    if (val == MTK_DISP_EVENT_BLANK && *blank == MTK_DISP_BLANK_UNBLANK) {
        queue_work(hyn_data->hyn_workqueue, &hyn_data->work_resume);
        return NOTIFY_OK;
    }
    return NOTIFY_DONE;
...
#else
    ts_data->mindone_disp_nb.notifier_call = mindone_disp_notifier_callback;
    ret = mtk_disp_notifier_register("hyn_ts", &ts_data->mindone_disp_nb);
    if (ret)
        HYN_ERROR("mtk_disp_notifier_register failed: %d", ret);
#endif
...
#else
        mtk_disp_notifier_unregister(&ts_data->mindone_disp_nb);
#endif
