...
static void spi_dump_config(struct spi_controller *master, struct spi_message *msg)
{
	...
	spi_debug("chip_config->chip_select:%d,chip_config->pad_sel:%d\n",
			spi_get_chipselect(spi, 0), mdata->pad_sel[spi_get_chipselect(spi, 0)]);
	spi_debug("||**************%s end**************||\n", __func__);
}
...
static int mtk_spi_probe(struct platform_device *pdev)
{
	...
	if (mdata->dev_comp->need_pad_sel) {
		...
		if (master->cs_gpiods) {
			/* gpiod core (spi_register_controller) already requests each
			 * line from "cs-gpios" DT property - just sanity-check presence,
			 * no separate devm_gpio_request() needed with the gpiod API.
			 */
