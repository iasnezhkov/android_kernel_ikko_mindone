...
//prize add by lipengpeng 20220615 start 
			...
			if (ret < 0) {
				...
				dai_link->num_codecs = ARRAY_SIZE(mindone_dummy_codec);
			}
#else
...
//prize add by lipengpeng 20220615 end
...

	{
		...
		pr_debug("MINDONE-LINK: total links %d\n", card->num_links);
		for_each_card_prelinks(card, li, dl) {
			pr_debug("MINDONE-LINK[%d] \"%s\": cpus=%d codecs=%d platforms=%d\n",
				li, dl->name ? dl->name : "?",
				dl->num_cpus, dl->num_codecs, dl->num_platforms);
			for (ci = 0; ci < dl->num_cpus; ci++)
				pr_debug("MINDONE-LINK[%d]   cpu[%d] name=%s dai=%s of=%pOF\n",
					li, ci, dl->cpus[ci].name ? dl->cpus[ci].name : "-",
					dl->cpus[ci].dai_name ? dl->cpus[ci].dai_name : "-",
					dl->cpus[ci].of_node);
			for (ci = 0; ci < dl->num_codecs; ci++)
				pr_debug("MINDONE-LINK[%d]   codec[%d] name=%s dai=%s of=%pOF\n",
					li, ci, dl->codecs[ci].name ? dl->codecs[ci].name : "-",
					dl->codecs[ci].dai_name ? dl->codecs[ci].dai_name : "-",
					dl->codecs[ci].of_node);
		}
	}
