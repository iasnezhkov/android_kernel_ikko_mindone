...
ashmem_vmfile_get_unmapped_area(struct file *file, unsigned long addr,
				...
				unsigned long flags)
{
	/* 6.12 dropped mm_struct::get_unmapped_area; mm_get_unmapped_area() is the
	 * direct replacement and, unlike get_unmapped_area(), does not dispatch
	 * through file->f_op, so overriding f_op here cannot recurse.
	 */
...
static int ashmem_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	/* shmem's own ->mmap, saved before we override the copy below: it is what
	 * installs the shmem vm_ops on the vma. 6.12 does not export
	 * shmem_zero_setup() to modules, and calling it was only ever a way to get
	 * those vm_ops - the file it attaches is replaced by vma_set_file() right
	 * after. Going through the backing file we already own needs no new export.
	 */
	static int (*shmem_orig_mmap)(struct file *, struct vm_area_struct *);
	struct ashmem_area *asma = file->private_data;
	int ret = 0;
	...
	if (!asma->file) {
		...
		if (!vmfile_fops.mmap) {
			...
			shmem_orig_mmap = vmfile->f_op->mmap;
	...
	if (vma->vm_flags & VM_SHARED) {
		ret = shmem_orig_mmap ? shmem_orig_mmap(asma->file, vma) : -EINVAL;
		if (ret) {
			fput(asma->file);
			goto out;
		}
...
#define ASHMEM_SHRINKER_SEEKS (DEFAULT_SEEKS * 4)
...
static void __exit ashmem_exit(void)
{
	...
	for (i = ARRAY_SIZE(ashmem_miscs) - 1; i >= 0; i--)
		misc_deregister(&ashmem_miscs[i]);
	kmem_cache_destroy(ashmem_range_cachep);
	kmem_cache_destroy(ashmem_area_cachep);
}
...
MODULE_DESCRIPTION("Anonymous Shared Memory Subsystem (ashmem)");
