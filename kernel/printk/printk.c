...
void __init setup_log_buf(int early)
{
	...
	/* MINDONE (F2438): the early log store's log_store() zeroes
	 * its 32-bit klog_addr field for any printk buffer physical address
	 * >= 4GiB instead of truncating it; with ~7.8GiB RAM, plain
	 * memblock_alloc() almost always lands above 4GiB. Prefer a
	 * DMA32-zone allocation, falling back to the old path if too tight. */
	new_log_buf = memblock_alloc_low(new_log_buf_len, LOG_ALIGN);
