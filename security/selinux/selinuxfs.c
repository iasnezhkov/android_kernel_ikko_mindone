...
/* MINDONE: block the transition into enforcing mode. Enabled by the
 * `mindone_permissive` cmdline param (no value). Needed because
 * `androidboot.selinux=permissive` is ignored by stock init on a user
 * build, and we need to test the "policy denial" class of boot failure. */
static int mindone_permissive;
static int __init mindone_permissive_setup(char *str)
{
	mindone_permissive = 1;
	pr_warn("MINDONE-SELINUX: enforcing mode BLOCKED by kernel param\n");
	return 1;
}
__setup("mindone_permissive", mindone_permissive_setup);
...

{
	...
	/* MINDONE: swallow the attempt to enable enforcing mode but report
	 * SUCCESS -- stock init treats a failure here as unrecoverable and
	 * aborts the boot. */
	if (mindone_permissive && new_value) {
		pr_warn("MINDONE-SELINUX: attempt to enable enforcing mode REJECTED\n");
		length = count;
		goto out;
	}
