...
/*
 * MTK android_vh vendor-hook port (2026-08-20).
 * Closes a symbol-closure gap for the out-of-tree mkp/ccmni/mtk_printk_ctrl
 * modules (the android_vh cluster).
 * The DECLARE_HOOK() declarations themselves were added to the
 * corresponding include/trace/hooks headers (creds.h, avc.h, module.h,
 * selinux.h gained android_vh_* siblings of their existing android_rvh_*
 * hooks; memory.h, logbuf.h, ipv6.h are new files), ported from a MediaTek
 * MT8781 reference tree (out-of-tree vendor 6.1) or, where the out-of-tree
 * module already carried an identical local copy of the header (memory.h
 * / mkp, logbuf.h / mtk_printk_ctrl, ipv6.h / ccmni), verified
 * byte-identical against that reference tree before being adopted as the
 * canonical in-tree version.
 *
 * CREATE_TRACE_POINTS is defined once at the top of this file (line 14)
 * and never undefined, so it is still in effect here; each #include below
 * instantiates (DEFINE_TRACE via trace/define_trace.h) the struct
 * tracepoint objects for its hooks, and the EXPORT_TRACEPOINT_SYMBOL_GPL
 * lines make them link-visible to out-of-tree modules.
 */
#include <trace/hooks/memory.h>
#include <trace/hooks/logbuf.h>
#include <trace/hooks/ipv6.h>

/* creds.h: android_vh_* siblings of the android_rvh_* hooks already exported above */
...
/* avc.h: android_vh_* siblings of the android_rvh_* hooks already exported above */
...
/* selinux.h: android_vh_* sibling of android_rvh_selinux_is_initialized */
EXPORT_TRACEPOINT_SYMBOL_GPL(android_vh_selinux_is_initialized);

/* module.h: android_vh_* siblings of the android_rvh_set_module_permit_* hooks */
...
/* memory.h: new file, ported verbatim (== mkp's own local trace/hooks/memory.h) */
...
/* logbuf.h: new file, ported verbatim (== mtk_printk_ctrl's own local trace/hooks/logbuf.h) */
EXPORT_TRACEPOINT_SYMBOL_GPL(android_vh_logbuf);
EXPORT_TRACEPOINT_SYMBOL_GPL(android_vh_logbuf_pr_cont);

/* ipv6.h: new file, ported verbatim (== ccmni's own local trace/hooks/ipv6.h) */
