...
const struct clk_ops mtk_mux_ops = {
	...
	/* 6.12: __clk_core_init rejects a mux (-22) unless it provides both
	 * .set_parent and .determine_rate. This vendor backport predates that
	 * requirement, so 53 clocks (axi_sel, spm_sel, disp_sel, cam_sel, ...)
	 * failed to register and every consumer of them went down with them. */
