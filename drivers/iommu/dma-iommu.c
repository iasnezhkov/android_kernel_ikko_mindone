...
/* MINDONE (F497): already present in vmlinux, just wasn't exported to
 * modules -- mtk_iommu.ko needs exactly this one. Body unchanged, export only. */
EXPORT_SYMBOL_GPL(iommu_get_dma_cookie);
...
/* MINDONE (F497): same story as
 * iommu_get_dma_cookie above -- body unchanged, export only. */
EXPORT_SYMBOL_GPL(iommu_put_dma_cookie);
...
void iommu_setup_dma_ops(struct device *dev, u64 dma_base, u64 dma_limit)
{
	...
	if (!domain) {
		/* MINDONE-IOMMU: domain is entirely absent -- exactly the F725
		 * case (the kernel freed it because the driver set up its own
		 * DMA cookie and the kernel got -EEXIST). */
		pr_warn("MINDONE-IOMMU: %s -- NO translation domain (domain == NULL)\n",
			dev_name(dev));
		goto out_err;
	}
	...
	if (iommu_is_dma_domain(domain)) {
		mnd_err = iommu_dma_init_domain(domain, dma_base, dma_limit, dev);
		if (mnd_err) {
			/* MINDONE-IOMMU: domain IS present but init failed -- a
			 * DIFFERENT cause than F725. The error code points at
			 * where to look next. */
			pr_warn("MINDONE-IOMMU: %s -- domain present, init failed, code %d "
				"(pgsize=0x%lx, cookie=%p)\n",
				dev_name(dev), mnd_err, domain->pgsize_bitmap,
				domain->iova_cookie);
			goto out_err;
		}
