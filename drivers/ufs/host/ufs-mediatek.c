...
/* MINDONE: mask out optional UFS quirk caps via a cmdline param. The stock
 * 5.10 kernel's vendor module logs `caps: 0x0` for this same controller (no
 * optional caps enabled); the in-tree 6.1 driver enables them unconditionally,
 * and kernel 6 hangs waiting on disk reads (init/apexd in
 * folio_wait_bit_common) -- testing whether these caps are the cause. Bits:
 * 1 clkgating, 2 autohibern8, 4 clkscaling, 8 write-booster, 16 crypto. */
static int mindone_ufs_nocaps;

static int __init mindone_set_ufs_nocaps(char *str)
{
	if (str)
		(void)kstrtoint(str, 0, &mindone_ufs_nocaps);
	return 1;
}
__setup("mindone_ufs_nocaps=", mindone_set_ufs_nocaps);
...
static int ufs_mtk_init(struct ufs_hba *hba)
{
	...
	if (!(mindone_ufs_nocaps & 2))
		hba->caps |= UFSHCD_CAP_RPM_AUTOSUSPEND;

	/* Enable clock-gating */
	if (!(mindone_ufs_nocaps & 1))
		hba->caps |= UFSHCD_CAP_CLK_GATING;

	/* Enable inline encryption */
	if (!(mindone_ufs_nocaps & 16))
		hba->caps |= UFSHCD_CAP_CRYPTO;

	/* Enable WriteBooster */
	if (!(mindone_ufs_nocaps & 8))
		hba->caps |= UFSHCD_CAP_WB_EN;

	/* Enable clk scaling*/
	if (!(mindone_ufs_nocaps & 4))
		hba->caps |= UFSHCD_CAP_CLK_SCALING;
