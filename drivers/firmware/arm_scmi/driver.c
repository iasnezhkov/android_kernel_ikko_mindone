...
/* MINDONE: bisection level 8 -- let SCMI firmware negotiation complete
 * FULLY, then abort the driver bind right after. Separates "the SCMI
 * exchange itself hangs the boot" from "something that comes up right
 * after SCMI succeeds hangs it" (mmdvfs, the display pipeline). The boot
 * outcome is the readout (F561-F567). */
static int mindone_probe_stop;

static int __init mindone_set_probe_stop(char *str)
{
	if (str)
		(void)kstrtoint(str, 10, &mindone_probe_stop);
	return 1;
}
__setup("mindone_scmi_probe_stop=", mindone_set_probe_stop);
...
static int scmi_probe(struct platform_device *pdev)
{
	...
	if (mindone_probe_stop) {	/* MINDONE: bisection level 8 */
		dev_info(dev, "MINDONE-SCMI: negotiation PASSED, aborting bind\n");
		ret = -ENODEV;
		goto notification_exit;
	}
