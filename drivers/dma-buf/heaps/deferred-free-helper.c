...
/* MINDONE: this helper comes from our 6.1 tree, where it still uses the pre-6.7 shrinker
 * API. register_shrinker() was replaced by the shrinker_alloc()/shrinker_register() pair,
 * so the registration is rewritten here; the callbacks themselves are unchanged. */
